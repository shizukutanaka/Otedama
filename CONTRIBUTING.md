...
Otedamaの開発には以下の環境が必要です。Go 1.25以上（`go.mod` の `go 1.25.0` ディレクティブ、`tlsmlkem` godebug による実質フロア）、Git、Docker（オプション、統合テスト用）、Make（ビルド自動化）、テキストエディタまたはIDE（VSCode、GoLand、Vim、Emacsなど任意）。クローンとビルドは以下のコマンドで実行できます。
...
Otedamaにプラグインアーキテクチャは存在しません。かつて「Otedama Marketplace」として計画されていましたが、プラグインエコシステムの需要証明が得られなかったため`ROADMAP.md`の「削除されたマイルストーン」に記録され、明示的に「やらないこと」となっています。新規収益源（`internal/provider/`）やハードウェア対応（`internal/hal/`）を追加したい場合は、コア本体へのPRとして提出してください。
