...
func setupWallet(opts Options, log func(level, msg string)) string {
	...
	if _, err := os.Stat(walletPath); errors.Is(err, os.ErrNotExist) {
		if out, ok := opts.Output.(*os.File); ok && !isTerminalFile(out) {
...
// isTerminalFile reports whether f is connected to an interactive
// terminal, as opposed to a redirected file, a pipe, or a service
// manager's log capture (systemd's journal, launchd's file-based stdout
// redirection). Stdlib-only: os.ModeCharDevice is set on a file's Stat()
// when the underlying descriptor is a character device, which a real
// terminal is and a regular file/pipe is not — same check as
// cmd/otedama's isTerminal.
