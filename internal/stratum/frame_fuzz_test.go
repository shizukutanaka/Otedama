...
func FuzzDecodeHeader(f *testing.F) {
	...
	seeds := [][]byte{
		...
		// Overflow-focused boundaries (SRI noise_sv2 overflow lesson):
		// U24 length at the exact maximum and one below it.
		{0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF},
		{0x00, 0x00, 0x00, 0xFE, 0xFF, 0xFF},
		// Channel message whose declared payload is below the
		// MinimumChannelPayload boundary (3 and 4 bytes).
		{0x00, 0x00, 0x00, 0x03, 0x00, 0x80},
		{0x00, 0x00, 0x00, 0x04, 0x00, 0x80},
		// Extension-type bit patterns in the top byte.
...
func FuzzDecoder_ReadFrame(f *testing.F) {
	seeds := [][]byte{
		...
		{0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF},
		// U24 length at the exact protocol maximum, followed by a small
		// body — the decoder must bound by header-declared length, not
		// wire length.
