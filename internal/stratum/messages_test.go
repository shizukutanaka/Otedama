...
func TestSetupConnection_Roundtrip(t *testing.T) {
	...
	if got.EndpointHost != orig.EndpointHost || got.EndpointPort != orig.EndpointPort {
		t.Errorf("Endpoint: got %q:%d, want %q:%d", got.EndpointHost, got.EndpointPort, orig.EndpointHost, orig.EndpointPort)
	}
...
func TestSetupConnection_EmptyStrings(t *testing.T) {
	...
	if got.EndpointHost != "" || got.EndpointPort != 0 || got.Vendor != "" {
		t.Errorf("empty strings not preserved: EndpointHost=%q Vendor=%q", got.EndpointHost, got.Vendor)
	}
}
...
func TestDecodeSubmitSharesSuccess_Basic(t *testing.T) {
	...
	binary.LittleEndian.PutUint64(buf[12:20], 5) // Summed (U64)
...
func TestDecodeOpenMiningChannelSuccess_TruncatedAtGroupChannelID(t *testing.T) {
	// Build a valid payload then chop into the last GroupChannelID bytes.
	orig := OpenMiningChannelSuccess{ReqID: 1, ChannelID: 1, Extranonce: []byte{0x01}, GroupChannelID: 4}
	payload, _ := orig.Encode()
	// Remove the last 2 bytes (GroupChannelID is uint32 — needs 4).
...
func TestDecodeSetupConnection_TruncatedLate(t *testing.T) {
	...
	payload, _ := orig.Encode()
	// Keep only enough bytes to get through EndpointHost+Port+Vendor+
	// HardwareVersion but cut off Firmware+DeviceID (remove last ~15 bytes).
...
// ----- Wire-layout conformance (ported checks for the 09fe275 defect class) -----
//
// Self-roundtrip tests cannot catch a missing/misaligned wire field — the
// encoder and decoder share the same bug. These tests assert the byte-level
// layout the SV2 spec mandates so the divergence class stays pinned.

func TestSetupConnection_WireLayout_EndpointPort(t *testing.T) {
	orig := SetupConnection{
		Protocol: MiningProtocol, MinVersion: 2, MaxVersion: 2,
		EndpointHost: "h", EndpointPort: 0xBEEF, Vendor: "v",
	}
	payload, err := orig.Encode()
	if err != nil {
		t.Fatalf("Encode: %v", err)
	}
	// Layout: proto(1)+minV(2)+maxV(2)+flags(4)+len(1)+"h"+port(2)+
	// len(1)+"v"+3×empty STR0_255 (hw/fw/device).
	want := []byte{
		0x00, 0x02, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x01, 'h', 0xEF, 0xBE, 0x01, 'v', 0x00, 0x00, 0x00,
	}
	if !bytes.Equal(payload, want) {
		t.Fatalf("wire layout = % X, want % X (endpoint_port U16 after host)", payload, want)
	}
}

func TestOpenMiningChannel_WireLayout_MaxTarget(t *testing.T) {
	orig := OpenMiningChannel{ReqID: 1, User: "u", NominalHashrate: 0, MaxTarget: MaxTargetAny()}
	payload, err := orig.Encode()
	if err != nil {
		t.Fatalf("Encode: %v", err)
	}
	// max_target is the mandatory last field (§5.3.2): U256 = 32 bytes.
	if len(payload) < 32+4+1+4 {
		t.Fatalf("payload too short for max_target: %d", len(payload))
	}
	for i, b := range payload[len(payload)-32:] {
		if b != 0xff {
			t.Fatalf("max_target byte %d = 0x%02x, want 0xff", i, b)
		}
	}
}

func TestOpenMiningChannelSuccess_WireLayout_GroupChannelID(t *testing.T) {
	// Last field on the wire is group_channel_id U32 (§5.3.3), not a U16.
	orig := OpenMiningChannelSuccess{ReqID: 1, ChannelID: 2, Extranonce: []byte{0xaa}, GroupChannelID: 0xDEADBEEF}
	payload, err := orig.Encode()
	if err != nil {
		t.Fatalf("Encode: %v", err)
	}
	got := binary.LittleEndian.Uint32(payload[len(payload)-4:])
	if got != 0xDEADBEEF {
		t.Fatalf("last U32 on wire = 0x%08x, want 0xDEADBEEF", got)
	}
	m, err := DecodeOpenMiningChannelSuccess(payload)
	if err != nil {
		t.Fatalf("Decode: %v", err)
	}
	if m.GroupChannelID != 0xDEADBEEF {
		t.Fatalf("decoded GroupChannelID = 0x%08x, want 0xDEADBEEF", m.GroupChannelID)
	}
}
