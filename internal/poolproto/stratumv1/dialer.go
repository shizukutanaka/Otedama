...
const optionalCallTimeout = 10 * time.Second
...
func (d *Dialer) Negotiate(ctx context.Context, c poolproto.Connection) (poolproto.Session, error) {
	...
	//
	// The call runs under a short timeout of its own: some pools (e.g.
	// public-pool.io) silently drop unknown methods instead of replying
	// "Method not found", so waiting on ctx would block Negotiate until
	// session shutdown — the engine would connect and never receive a job.
	// Timing out here only loses the optional capability announcement.
	id = sess.nextID.Add(1)
