...
- **In effect (session 256):** `go.mod` now pins `go 1.25.0` /
  `toolchain go1.25.13` — the container-aware default is compiled
  into current builds. It was previously "documented but not compiled
  in" (session 251 note); the bump also closed the reachable
  stdlib advisories govulncheck reported under the 1.24-line
  toolchains (net/http, crypto/tls, crypto/x509 — see
  docs/THREAT_MODEL.md dependency assumptions).
- Otedama impact: fixes a class of "miner saturates noisy-neighbor
  pod limit" reports we expect from Kubernetes users.
