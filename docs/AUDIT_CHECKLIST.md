...
| 13 | Dependabot enabled for Go, Actions, Docker | `.github/dependabot.yml`; the repository's pull requests | read the file, then search PRs by `author:app/dependabot` | **CONFIGURED, NOT SHOWN WORKING** — the file lists all three ecosystems weekly, but **Dependabot has never opened a pull request in this repository** (GitHub search, 2026-09-24: 0 results). This row said PASS on the strength of the file alone, which is the same "configured ≠ working" error this checklist exists to catch. Master's PR #6 also added an `automerge:` key under the `github-actions` entry; whether Dependabot's config schema accepts that key could not be checked from the verifying environment (docs.github.com is blocked there) — **調査が必要**. If the key is not in the schema, Dependabot reports a config error and runs no updates for any ecosystem, which would be consistent with the zero PRs, but zero PRs predates PR #6 as well, so it is not evidence either way |
