...
| `shares_submit_dropped_total` | counter | Shares discarded at the engine without transmission because the concurrent-submit budget (64 in-flight V1 calls) was saturated — non-zero only under a pool-driven share flood far above any legitimate rate. |
