...
4. **Reconnect loop** (`runReconnectLoop`):
   ...
   - For each attempt: dial TCP → Stratum V2 handshake (SetupConnection +
     ...
     and cannot process extended/group jobs, per sv2-spec §5.3.1; the
     handshake fails when SetupConnectionSuccess.flags requires extended
     channels) → on success the address is marked *known-good*.
...
| `shares_dropped_total` | counter | Shares found locally but discarded because the worker's share channel was full at emit time — discovered-but-lost work. |
| `shares_total{status}` | counter | Shares judged by the pool (`accepted`/`rejected`). |
| `shares_rejected_by_reason_total{reason}` † | counter | Rejects by inferred cause (stale/duplicate/difficulty/hardware/other/difficulty-change). `difficulty-change` = benign post-set_difficulty race rejects, excluded from reject_rate. |
...
| `submit_latency_seconds` | histogram | submit→verdict RTT in canonical seconds (le buckets 0.01–10). Each bucket sample may carry an OpenMetrics-style exemplar `# {share_seq="N"}` (V2) or `# {job_id="N"}` (V1) linking the observation to the share that produced it; the ` # {…}` syntax degrades to a comment on text/0.0.4 parsers. |
