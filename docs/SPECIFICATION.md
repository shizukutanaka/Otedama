...
| `shares_total{status}` | counter | Shares judged by the pool (`accepted`/`rejected`). `accepted` sums the pool-reported `new_submits_accepted_count` (per-batch per SV2 §5.3.13); `rejected` counts per-share errors. |
