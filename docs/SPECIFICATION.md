...
| `shares_pending` | gauge | Submitted but not yet judged (submitted−accepted−rejected−difficulty-transition-rejected−submit-failed, ≥0) — the shares the pool owes a verdict. SV2 pools batch acks, so a non-zero baseline is normal there (ESP-Miner #1735). unaccounted = pending + found-but-never-submitted + submit-failed. |
...
| G3 | Engine bypasses the `poolproto` dialer abstraction (inline handshake). | **Fixed (session 259)**: the engine's ~600-line inline V2 handshake/read/submit loop is deleted; `runSession` dispatches every scheme to `poolproto.DialURL` and both protocols share one session loop (`runPoolSession`). The dialer was completed first — real v2tls:// TLS dial, `OpenMiningChannelSuccess` initial target, `SetTarget` handling, and sequence-correlated share verdicts (Submit waits for `SubmitSharesSuccess`/`Error` instead of a provisional accept) — so no telemetry regressed. The submit path now additionally surfaces the share's hashed header `version` to the pool. |
