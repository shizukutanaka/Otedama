...
- `lightning.WalletManager.Seed/Mnemonic/ChangePassphrase`,
  ...
  leave untouched. (Update, session 255: `ChangePassphrase` and
  `MnemonicToEntropy` are now live callers' dependencies — wired by
  `otedama wallet verify` / `otedama wallet change-passphrase`.)
...
| S | Package doc claimed terminal width is "detected at startup via TIOCGWINSZ (Unix) or GetConsoleScreenBufferInfo (Windows)"; `SetWidth` exists but is called only from test files — every real invocation renders at the hardcoded 80-column default regardless of actual terminal size. | ✅ Resolved (session 255): real detection landed via the `x/sys` path (`internal/tui/winsize*.go`) — TIOCGWINSZ on Unix-likes, GetConsoleScreenBufferInfo on Windows, per-tick re-detection for resizes. No new module dependency (`x/sys` was already indirect via `x/crypto`); ADR-003 preserved. KNOWN_LIMITATIONS §15 closed. |
