...
## Session 308 update — httpserver lifecycle + method scope; small-package sweep clean

| Finding | Disposition |
|---|---|
| **`Start`→`Stop`→`Start` (and `Stop`→`Start`) reported a dead server as live** — `http.Server` cannot serve after `Shutdown`, yet a second `Start` bound a fresh listener that `Serve` dropped instantly (`ErrServerClosed`, filtered as expected), leaving `boundAddr` pointing at a port nothing listens on — the same lifecycle class as s300's TUI Start-after-Stop | ✅ `started atomic.Bool`: `Start` CAS-rejects a second call; `Stop` also marks it so Stop-before-Start is rejected too. All orderings now error instead of reporting a zombie. |
| mux accepted every method on probe/scrape routes — POST/DELETE to `/readyz`/`/healthz`/`/metrics` returned health-looking 200/503 bodies | ✅ Go 1.22+ method-scoped patterns (`GET /healthz`, `GET /readyz`, `GET /metrics`, `GET /`, `GET /debug/pprof/*`) — the mux now answers non-GET with 405 structurally. |
| `Server.Addr()` / `Server.ServeError()` have no non-test callers — exported observability helpers deadcode flags | 📋 Recorded: kept for supervisor/health-check integration; pre-existing deadcode noise, unchanged by this session. |
| `handleMetrics` swallows `WriteText` errors after status 200 is already sent | 📋 Recorded: nothing actionable — status is committed; package has no logger to report through. |
| Audited clean — `internal/httpserver` remainder (slowloris timeouts, boundAddr CAS, ctx-watcher Shutdown, 404-on-unknown-path), `internal/i18n/message.go` (immutable catalogs, base-tag→English fallback, graceful template degradation, `MissingTranslations` completeness), `internal/clock` (documented non-monotonicity, race-free Fake), `internal/version` (ldflags vars + Info), `internal/logger` (CAS default singleton, nil-safe context helpers) | ✅ No change needed. |
| Tests: `TestStart_AfterStop_ReturnsError`, `TestStart_AfterStopWithoutStart_ReturnsError`, `TestMethodNotAllowed_OnProbes` | ✅ All green; 24 packages pass; vet/gofumpt/deadcode report nothing new. |

## Session 309 update — arbitration merge determinism + stratum/provider remainder

| Finding | Disposition |
|---|---|
| **`applyAllocation` logged "N idle (reason)" every 30s tick forever** — a device held idle by the min_yield floor accumulated thousands of identical lines/day | ✅ `idleSeen` (device→reason) dedup wired from `runArbitrationLoop`; reason changes and idle→assigned→idle round trips still log. Variadic param keeps existing call sites compatible. |
| **`streamsSlice` representative pick depended on Go map iteration order** — a provider emitting both a wildcard ("providerID:") and per-device ("providerID:dev") quotes could promote either entry's `DefaultYield` nondeterministically; result order also unstable | ✅ Sorted key iteration: wildcard key sorts first and deterministically becomes the representative (provider-declared default is the sane fallback); per-device yields still merge in; output order stable. |
| `Frame.Payload` doc claimed the decoder may reuse the buffer and callers must not retain it — contradicted `ReadFrame`'s "freshly allocated, caller owns" (the implementation) | ✅ Doc corrected: payload is per-frame `make([]byte, MsgLength)`; only the 6-byte header scratch is reused and never aliases Payload. |
| `pollingProvider.Quotes()` reads `p.quoteCh` without the mutex while `Stop()` swaps the field under it | 📋 Recorded: data race exists only if `Quotes()` is called concurrently with `Stop()` — engine fetches the channel once before `Start` (sequential). Contract "Start once" makes restart path dormant. |
| `pollingProvider.Stop` "resets state so the provider can be started again" vs `Provider.Start` doc "must only be called once" | 📋 Recorded: implementation supports restart, interface contract is conservative — kept as-is; consumers may rely on single-start. |
| `AkashProvider.publish` substitutes a hardcoded `rate = 95000` when `BTCUSDRate() <= 0` — silently fabricates the fiat conversion on a dead feed; `confidence` still follows `fresh` so a `(0, true)` source yields fabricated sats/s at 0.85 | 📋 Recorded: provider is a fixed-price simulation end to end ("(simulated)" suffix is load-bearing); honest fix (Confidence 0 like the no-GPU path) noted for the real-API integration in ADR-013. |
| `updateStream` `net <= 0 → net = gross` fallback reads an explicit `Net=0` (fee ate everything) as "no explicit fee" and inflates to gross | 📋 Recorded: edge needs a provider that actually reports 100% fee; Yield doc defines Net unset = gross fallback. |
...
## Session 311 update — config-file load failure conflated with missing file

| Finding | Disposition |
|---|---|
| **`loadConfigFile` treated "file present but unparseable/unreadable" the same as "file absent"** — a stderr warning plus a zero `config.Config` meant `otedama run` silently dropped the operator's pools/bitcoin_address/economics onto defaults, and `config validate` reported the misleading `bitcoin_address is required` instead of the real parse error | ✅ `loadConfigFile` now returns `(Config, error)`; `run`, `config show`, and `config validate` exit `exitConfig(78)` with the load error. Missing file stays a clean no-error path. |
| **`checkConfig` passed `os.Stat` on a present-but-broken file** — doctor could report "loaded from path" (Pass) for a file that was never actually parsed (once cfg validated via env/flags), and the only evidence was a stderr warning invisible to `--json` consumers | ✅ `DefaultChecks`/`checkConfig` take a `loadErr` and report the load failure as a Fail verdict — it now surfaces in text and JSON reports alike. |
| `DifficultyFromTarget` can return `+Inf` for targets below ~diff1·2⁻¹⁰⁷⁴ (absurd share target) — propagates to `SuggestedDifficulty()` display/metric | 📋 Recorded: a pool assigning an unreachable target is already a broken-session symptom; displayed "+Inf" is cosmetic, not silent. |
| `defaultConfigPath` hardcodes `~/.config/otedama/` — on Windows `%APPDATA%` is the conventional location (mixed-separator path still resolves) | 📋 Recorded: documented default; consistent with data_dir's `~/.local/share` convention; changing it would orphan existing installs' files. |
| Audited clean — `internal/miner/coinbase.go` (fold order + header byte order vs ESP-Miner), `sha256d.go` (nBits sign bit, zero-mantissa/overflow rejects, fractional difficulty via big.Float, diff1 constant), `worker.go` remainder (batched hashCount stays exact on share-flush/early-break paths, nonce-lane exhaustion parks, SetPaused clears work, Start-once panic), `cmd/otedama/completion.go` (bash/zsh/fish command lists match dispatch table exactly), `version.go`, `winsvc_*` | ✅ No change needed. |
| Tests: `TestLoadConfigFile_MalformedYAML_ErrorsAndReturnsEmpty`, `TestCheckConfig_LoadError_FailsEvenWithValidConfig`, `TestCmdConfigValidate_BrokenFile_ExitsConfig`, `TestCmdConfigShow_BrokenFile_ExitsConfig`, `TestCmdDoctor_BrokenFile_ReportsFailInJSON`; ~14 call sites updated for the new signatures | ✅ All green; 24 packages pass; vet/gofumpt/deadcode report nothing new. |

## Session 312 update — TUI ANSI-width padding remainder; metrics exposition audit

| Finding | Disposition |
|---|---|
| **`miningLine`/`earningsLine` carried the same `% -Ns`-with-ANSI defect s296 fixed in `poolLine`** — width-verbatim Sprintf counts escape bytes, so the device column and the sats/day total never padded and the shares/est fields slid left (cosmetic only; mid-line so no truncation) | ✅ Visible-width padding applied identically to poolLine; regression test pins the shares column at visible offset 40 (default), 50 (stalled), 51 (paused) and the est. earned column at 34. |
| `RegisterCollector` has no duplicate-registration guard — a second registration of the same collector emits duplicate `# TYPE` lines, which Prometheus rejects (the whole scrape becomes unparseable) | 📋 Recorded: funcs aren't comparable without reflect; single call site today, developer-error class consistent with the package's panic-on-bad-registration contract. |
| `shortenURL`/`truncateToBudget` cut at BYTE offsets — a multibyte rune in a pool URL or share text could be split mid-sequence and emit invalid UTF-8 | 📋 Recorded: pool URLs are ASCII by nature (IDN is punycode); cosmetic-only, same class as all byte-budget truncations. |
| `miningLine` stalled/paused branches shift the shares column right by the badge width (by design — the badge is longer than the devs column) | 📋 Recorded: intentional — connection-state badges take layout priority; columns realign per state deterministically. |
...
## Session 315 update — stratumv1 parse/dialer/tls remainder audit

| Finding | Disposition |
|---|---|
| `parseNotify` swallowed parse failures of **required** fields (`version`/`nbits`/`ntime` hex, `prevhash` 32-byte decode, `coinb1`/`coinb2` hex) via `err == nil` guards — a malformed notify propagated a Job with zeroed NBits/PrevHash that the worker then ground shares against (guaranteed pool-side rejects; silent corruption vs loud drop — same class as the s311 broken-config fix) | ✅ Required-field errors now drop the notify; deliberate tolerances kept (clean_jobs 0/1 encoding, per-entry merkle_branch skips). Regression test covers all six fields + wrong-length prevhash. |
| `parseDifficulty` accepted any JSON number including `0`, negatives, `1e400` (+Inf), and non-finite values — a malformed `mining.set_difficulty` overwrote the live share filter | ✅ `!(d > 0) || IsInf` rejected (NaN excluded by the same check); prior valid difficulty retained on drop. |
| `parseSetExtranonce` returns `en1` unvalidated (non-hex would later fail coinbase build) | 📋 Recorded: downstream coinbase assembly rejects bad hex loudly; session clamps `sz` post-s301. |
| `parseAddress` leaves path/query suffixes in the dial target (`stratum+tcp://h:1/x` → `h:1/x`) | 📋 Recorded: fails fast with a clear dial error; config-layer URL validation is the right place for it. |
| Verified clean — `tls.go` (verify-always, custom CA = additive trust that fails closed on empty pool, TLS1.2 floor, bounded handshake 2×15s), `dialer.go` (per-pool CA precedence: injected cfg > creds bundle > system roots; optional `extranonce.subscribe` bounded 10s post-s304; authorize echo worker fix post-s303; `connection` closeOnce idempotent), `stratumv1.go` remainder (extranonce2 clamp post-s301, notice drain post-s298, id-request replies post-s307, rpcTimeout post-s307) | ✅ No change needed. |
| Tests: `TestParseNotify_MalformedRequiredFields`, `TestParseDifficulty_RejectsNonPositiveAndNonFinite` | ✅ All green; 24 packages pass; vet/gofumpt report nothing new. |

## Session 316 update — internal/stratum remainder + engine stats/metrics/setup + config resolve audit

| Finding | Disposition |
|---|---|
| `rejectByReason` lazy counter map was the only sibling without a mutex — `lastRejectByReason`/`sharesFoundPerDevice`/`payoutInfo` all carry dedicated mutexes, but `rejectReason` (map insert) and `updateShareRates` (two map reads) were unguarded. Safe only because every caller currently runs on the session goroutine; any future concurrent reject recorder would hit a fatal concurrent-map read/write | ✅ `rejectByReasonMu` added; `rejectReason`/`updateShareRates` now guard under the same pattern. Engine suite green under `-race`. |
| `detector.Detect` partial failure: per-driver errors are surfaced via the log callback and a non-empty device list silently swallows the aggregate `err` | 📋 Recorded: degraded-detection is by design (warn per driver, run with what enumerated). |
| Verified clean — `internal/stratum/tls.go` (mirrors V1: verify-always, additive CA, TLS1.2 floor, bounded handshake), `wire.go` (bounded STR0_255/B0_255 decoders; documented B0_32 Postel leniency; LE encoding), `messages.go` (all decoders length-check before reads; `min_ntime` OPTION 0/1 count + bounds; `DispatchFrame`→`UnknownMessage` forward-compat) | ✅ No change needed. |
| Verified clean — `internal/engine/stats.go` (windowed hashrate saturate-on-reset, uptime/sats accountants carry sub-second remainders, LatencyTracker rejects NaN samples, nearest-rank quantile clamps, HashrateMonitor reset semantics), `setup.go` (nonce-lane partition `NonceBase=i*Threads`/`NonceStep=D*Threads` correct for shared Work; wallet-mint non-TTY guard post-s305; payoutAddresses dedup; cpuDriver), `config.go` Resolve/EnvWarnings/FlagValues (4-layer precedence consistent; no numeric flags exist so FlagValues covering strings-only is by design; non-finite env rejected post-s296; Layer-4 DataDir default keeps OriginDefault) | ✅ No change needed. |
| All 24 packages pass (`go test ./...`); `go vet`/`gofumpt`/`deadcode` report nothing new | ✅ Gates green. |

## Session 317 update — internal/stratum audit completion (handshake/frame/noise)

| Finding | Disposition |
|---|---|
| `ReadMessage2`'s x-only fallback completes the handshake with **no DH at all** (`hs.mixHash(payload[:32]); hs.complete = true`) — transport keys derive from public data only | 📋 Recorded: already covered by KNOWN_LIMITATIONS §2 item 3 + inline marker; maintainer-gated rework (ADR-011). No new action. |
| Responder static key is never decrypted or authenticated (`remoteStatic` never populated) — the defining property of the NX pattern | 📋 Recorded: covered by KNOWN_LIMITATIONS §2 item 3 ("no code anywhere authenticates a responder static key"). |
| `EncryptedConn.Read` silently absorbs empty-plaintext frames: a decrypted zero-length payload leaves `readbuf` empty, so Read loops to the next frame inside a single call — a peer sending continuous empty frames can make `Read` never return (bounded only by the peer) | 📋 Recorded: maintainer-gated stub, no live callers; belongs to the same noise rework. |
| `CipherState.key` and `HandshakeState.localEphemeral`/`remoteStatic` are never zeroed after use | 📋 Recorded: secret-hygiene class (s277/s306 pattern); latent — the code is unwired. |
| `EncryptedConn.Write` trusts `io.Writer` contract for the ciphertext write and reports `len(p)` regardless | 📋 Recorded: correct per io.Writer contract (short write must error); noted only. |
| `CipherState.n` has no 2^64 rollover guard (Noise requires rekey before wraparound) | 📋 Recorded: unreachable in practice (~584 years at 1M msgs/s). |
