...
with non-terminal stdout the engine deliberately does not mint a wallet —
the one-time recovery phrase could only reach a log sink nobody reads,
leaving an unbacked-up wallet with a persisted secret. Create the wallet
once by running interactively (`otedama run --bitcoin-address …
--wallet-passphrase …`, record the phrase), then let the service unlock
the existing `wallet.dat`.
