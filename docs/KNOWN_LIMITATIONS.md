...
- **`ci-cd.yml`** is a second, largely duplicate "CI/CD Pipeline"
  ...
  matrix, both below `go.mod`'s `go 1.26` minimum (so those legs cannot
...
- **Go-version mismatch breaks EVERY Go job at `go.mod` parse time
  ...
  declares `go 1.26.0` + `toolchain go1.26.8` and — decisively — a
  `godebug` block containing `tlsmlkem=1`, which is a **Go 1.24+** knob
  (X25519MLKEM768, standardized in 1.24). Go 1.23/1.21 with
  `GOTOOLCHAIN=local` refuses to download the newer toolchain and fails
  immediately at the very first `go mod download` step (`unknown
  godebug "tlsmlkem"` on Go ≤1.23; `requires go >= 1.26.0` on 1.24–1.25)
  — so the Test, Build, Lint, Benchmark, and gosec jobs never even
  compile the code. This is not a code defect; the module is internally
  consistent for Go 1.26+ (it builds and passes all packages' tests
  locally on go1.26.8). It is purely that CI pins a Go older than the
  module's own floor. Note the latent tension it exposes, now resolved
  in favor of honesty: GODEBUG_NOTES.md used to claim the `go`/`toolchain`
  split let "older toolchains still build Otedama," but the `tlsmlkem=1`
  godebug (a 1.24+ knob) already made `go.mod` unparseable by any
  toolchain < 1.24 — so the session-255 bump to `go 1.26.0` /
  `toolchain go1.26.8` (driven by `golang.org/x/crypto v0.57.0`'s
  `go 1.26` floor and the GOMAXPROCS/container fix in GODEBUG_NOTES.md)
  merely records the constraint that already existed.
