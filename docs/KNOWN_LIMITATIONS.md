...
- **Go-version mismatch breaks EVERY Go job at `go.mod` parse time
  ...
  previously described a `go 1.22` / `toolchain go1.24.0` split
  existing so "older toolchains can still build Otedama" — but the
  `tlsmlkem=1` godebug (a 1.24 knob) already made `go.mod` unparseable
  by any toolchain < 1.24, so that stated intent was never actually
