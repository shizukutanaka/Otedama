...
**Otedamaの判断:**
- `go 1.25.0` をベースライン、`toolchain go1.25.13` を最低toolchain pin（session 256 で四半期policyに従い更新: container-aware GOMAXPROCS 有効化 + `net/http`/`crypto/tls`/`crypto/x509` の reachable stdlib advisory を解消。旧: `go 1.22`/`go1.24.0`）。
