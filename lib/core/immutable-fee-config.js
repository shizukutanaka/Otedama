...
const IMMUTABLE_CONFIG = {
  ...
  OPERATOR_ADDRESSES: {
    mainnet: 'bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh', // Example mainnet address
    testnet: 'tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx'  // Example testnet address
  },
...
class ImmutableFeeConfig {
   ...
   */
  getOperatorAddress(network = 'mainnet') {
    this.enforceIntegrity();
    
    const address = this.config.OPERATOR_ADDRESSES[network];
    if (!address) {
      throw new Error(`No operator address configured for network: ${network}`);
    }
   ...
   */
  calculateFeeDistribution(feeAmount) {
    this.enforceIntegrity();
...
module.exports = Object.freeze({
  ...
  getOperatorAddress: (network) => immutableFeeConfig.getOperatorAddress(network),
