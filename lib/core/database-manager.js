...
class TransactionManager {
  ...
  begin(isolationLevel = null) {
    ...
    try {
      if (isolationLevel) {
        this.connection.db.exec(`BEGIN ${isolationLevel}`);
      } else {
  ...
  rollbackTo(name) {
    ...
    try {
      this.connection.db.exec(`ROLLBACK TO ${name}`);
...
export class DatabaseManager extends EventEmitter {
  ...
  async getConnection(readonly = true) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.waitQueue.push({ readonly, resolve, reject, timeout });
    });
  }
  ...
  releaseConnection(connection) {
    ...
    const waiterIndex = this.waitQueue.findIndex(w => w.readonly === connection.readonly);
    if (waiterIndex !== -1) {
