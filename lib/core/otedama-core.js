...
// Lazy-loaded core modules for memory optimization
let MiningPoolOptimized, getDEX, DeFiCore, P2PNetworkOptimized;
let SecurityManager, PerformanceOptimizer, DatabaseManager, getLogger;

// Dynamic imports for better memory management
const loadCoreModules = async () => {
  if (!MiningPoolOptimized) {
    const modules = await Promise.all([
      import('../mining/pool-optimized.js'),
      import('../dex/index.js'),
      import('../defi/core.js'),
      import('../p2p/network-optimized.js'),
      import('../security/manager.js'),
      import('../performance/optimizer.js'),
      import('../database/manager.js'),
      import('../logger.js')
    ]);
    
    MiningPoolOptimized = modules[0].MiningPoolOptimized;
    getDEX = modules[1].getDEX;
    DeFiCore = modules[2].DeFiCore;
    P2PNetworkOptimized = modules[3].P2PNetworkOptimized;
    SecurityManager = modules[4].SecurityManager;
    PerformanceOptimizer = modules[5].PerformanceOptimizer;
    DatabaseManager = modules[6].DatabaseManager;
    getLogger = modules[7].getLogger;
  }
};
...
export class OtedamaCore extends EventEmitter {
  constructor(config = {}) {
    ...
    // Core state with WeakMaps for better garbage collection
    this.state = {
      initialized: false,
      running: false,
      startTime: null,
      ...
      workers: new Map(),
      metrics: new Map(),
      connections: new WeakMap(), // Use WeakMap for automatic memory cleanup
      cache: new Map() // Add cache with size limits
    };
    
    // Set cache size limit
    this.maxCacheSize = CORE_CONSTANTS.CACHE_SIZE;
    
    // Performance tracking with circular buffer for memory efficiency
    this.perf = {
      startTime: performance.now(),
      frameTime: 0,
      avgFrameTime: 0,
      frameCount: 0,
      history: new Float32Array(1000), // Fixed-size buffer
      historyIndex: 0,
