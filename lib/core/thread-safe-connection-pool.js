...
const logger = getLogger('ThreadSafeConnectionPool');
 ...
 */
export class ThreadSafeConnectionPool extends EventEmitter {
  constructor(options = {}) {
    super();
  ...
  async _createConnectionSafe() {
    ...
    try {
      ...
      try {
        ...
        const connection = new SafeConnection(id, socket);
...
export function createThreadSafeConnectionPool(options) {
  return new ThreadSafeConnectionPool(options);
}

export default ThreadSafeConnectionPool;