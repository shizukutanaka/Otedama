 ...
 * Unified lightweight logging implementation
 * 
 * Design: Simple, efficient logging (Carmack/Pike principles)
 */
  ...
  const logger = winston.createLogger({
    ...
    format: winston.format.combine(
      ...
      winston.format.printf(({ level, message, timestamp, stack }) => {
        return `${timestamp} [${name}] ${level.toUpperCase()}: ${stack || message}`;
      })
    ),
    transports: [
