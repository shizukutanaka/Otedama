...
export class ConfigManager extends EventEmitter {
  ...
  async loadFromFile() {
    try {
      ...
      if (stat.isFile()) {
        ...
        // The import will always get the latest version when using dynamic imports
        
        // Import config file (convert to file URL for Windows compatibility)
        const fileUrl = pathToFileURL(configPath).href;
        const { default: fileConfig } = await import(fileUrl);
  ...
  setAll(config) {
    if (!config || typeof config !== 'object') {
      throw new ConfigurationError('Invalid configuration object');
    }
    
    // Deep merge the config into overrides
    ...
    deepMerge(this.overrides, config);
