  ...
  validateCrossFields() {
    ...
    if (this.config.NODE_ENV === 'production') {
      ...
      const weakSecrets = [
        ...
        'default', 'secret', 'password', 'changeme',
        '12345678', '00000000', 'admin123', 'test1234'
      ];
      
      if (!this.config.SESSION_SECRET || weakSecrets.some(weak => 
        this.config.SESSION_SECRET.toLowerCase().includes(weak.toLowerCase())
      )) {
        this.errors.push('SESSION_SECRET must be a strong, random value in production');
      }
      
      if (!this.config.JWT_SECRET || weakSecrets.some(weak => 
        this.config.JWT_SECRET.toLowerCase().includes(weak.toLowerCase())
      )) {
        this.errors.push('JWT_SECRET must be a strong, random value in production');
      }
      
      // Ensure secrets are different
      if (this.config.SESSION_SECRET === this.config.JWT_SECRET) {
        this.errors.push('SESSION_SECRET and JWT_SECRET must be different values');
      }
      
      // Check CSRF secret if present
      if (this.config.CSRF_SECRET && weakSecrets.some(weak => 
        this.config.CSRF_SECRET.toLowerCase().includes(weak.toLowerCase())
      )) {
        this.errors.push('CSRF_SECRET must be a strong, random value');
      }
    }
