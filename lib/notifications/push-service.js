...
export class PushNotificationService extends EventEmitter {
    ...
    async initializeServices() {
        try {
            ...
            logger.error('Failed to initialize push services:', error);
        }
    }
    ...
    async registerDevice(userId, deviceInfo) {
        ...
        logger.info(`Device registered for user ${userId}:`, { platform, deviceId });
    ...
    async unregisterDevice(userId, deviceId) {
        ...
        logger.info(`Device unregistered for user ${userId}:`, { deviceId });
    ...
    async sendNotification(userId, notification, options = {}) {
        ...
        if (!userDevices || userDevices.size === 0) {
            logger.debug(`No devices registered for user ${userId}`);
    ...
    async handleInteraction(userId, notificationId, action) {
        logger.info('Notification interaction:', { userId, notificationId, action });
