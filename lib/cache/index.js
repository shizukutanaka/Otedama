...
// Export the efficient LRU cache for direct use
export { EfficientLRUCache, LRUCacheFactory, createLRUCache } from './efficient-lru-cache.js';
...
import { LRUCacheFactory } from './efficient-lru-cache.js';
...
export const CacheFactory = {
  ...
  createPoolCache: (options = {}) => createCache('pool', { useEfficientLRU: true, ...options }),
  
  // DEX cache with very short TTL and pattern analysis
  createDexCache: (options = {}) => createCache('dex', { useEfficientLRU: true, ...options }),
  
  // DeFi cache with medium TTL
  createDefiCache: (options = {}) => createCache('defi', { useEfficientLRU: true, ...options }),
  
  // API response cache with configurable TTL
  createApiCache: (options = {}) => createCache('api', { useEfficientLRU: true, ...options }),
  
  // Session cache with long TTL
  createSessionCache: (options = {}) => createCache('session', { useEfficientLRU: true, ...options }),
  
  // Generic cache with custom options
  createCustomCache: (options = {}) => new UnifiedCacheManager({ useEfficientLRU: true, ...options }),
