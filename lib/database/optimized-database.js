...
class OptimizedDatabase {
  ...
  async getMiner(address) {
    ...
    return this._timedQuery(() => this.pool.get(sql, [address]), 'getMiner');
  }
  ...
  async createMiner(address, workerName = null) {
    ...
    await this._timedQuery(
      () => this.pool.run(sql, [id, address, workerName]),
      'createMiner'
    );
  ...
  async recordShare(minerId, jobId, nonce, hash, difficulty, isValid = true) {
    ...
    return this._timedQuery(
      () => this.pool.run(sql, [minerId, jobId, nonce, hash, difficulty, isValid ? 1 : 0]),
      'recordShare'
    );
  }
  ...
  async getRecentShares(minerId, hours = 24) {
    ...
    return this._timedQuery(
      () => this.pool.execute(sql, [minerId, since]),
      'getRecentShares'
    );
  }
  ...
  async recordBlock(height, hash, previousHash, minerId, reward) {
    ...
    const result = await this._timedQuery(
      () => this.pool.run(sql, [height, hash, previousHash, minerId, reward]),
      'recordBlock'
    );
  ...
  async updateBlockConfirmations(blockId, confirmations) {
    ...
    return this._timedQuery(
      () => this.pool.run(sql, [confirmations, blockId]),
  ...
  async createPayment(minerId, amount, fee = 0) {
    ...
    return this._timedQuery(
      () => this.pool.run(sql, [minerId, amount, fee]),
      'createPayment'
    );
  }
  ...
  async processPayment(paymentId, transactionId) {
    ...
    return this._timedQuery(
      () => this.pool.run(sql, [transactionId, paymentId]),
      'processPayment'
    );
  }
  ...
  async updateBalance(minerId, confirmed = 0, pending = 0) {
    ...
    return this._timedQuery(
      () => this.pool.run(sql, [minerId, confirmed, pending]),
      'updateBalance'
    );
  }
  ...
  async recordMetric(type, name, value, tags = null) {
    ...
    return this.pool.run(sql, [type, name, value, tagsJson]);
  }
  ...
  async getMetrics(type, name, since, until = Date.now()) {
    ...
    return this._timedQuery(
      () => this.pool.execute(sql, [type, name, since, until]),
      'getMetrics'
    );
  }
  ...
  async vacuum() {
    ...
    await this.pool.vacuum();
  }

  async checkpoint() {
    await this.pool.checkpoint();
  }

  async getStats() {
    const [miners, shares, blocks, payments] = await Promise.all([
      this.pool.get('SELECT COUNT(*) as count FROM miners'),
      this.pool.get('SELECT COUNT(*) as count FROM shares WHERE timestamp > ?', [Date.now() - 86400000]),
      this.pool.get('SELECT COUNT(*) as count FROM blocks'),
      this.pool.get('SELECT COUNT(*) as count, SUM(amount) as total FROM payments WHERE status = "completed"')
    ]);
  ...
  async cleanup(daysToKeep = 7) {
    ...
    const results = await this.transaction(async () => {
      const shares = await this.pool.run(
        'DELETE FROM shares WHERE timestamp < ?',
        [cutoff]
      );
      
      const metrics = await this.pool.run(
