...
export class SecurityManager extends EventEmitter {
  ...
  // SECURITY FIX: Credential validation using environment variables
  async validateCredentials(credentials) {
    ...
    if (!adminUsername || !adminPassword) {
      ...
      throw new Error('Admin credentials not configured properly');
    }
    
    // Use timing-safe comparison to prevent timing attacks
    const usernameValid = this.timingSafeCompare(
      Buffer.from(credentials.username || '', 'utf8'),
      Buffer.from(adminUsername, 'utf8')
    );
    
    const passwordValid = this.timingSafeCompare(
