...
export class CSRFProtection extends EventEmitter {
    ...
    /**
     * Encrypt data using AES-256-GCM (secure authenticated encryption)
     */
    encrypt(data) {
        try {
            ...
            const key = createHash('sha256').update(this.options.secret).digest();
            ...
            this.emit('error', { phase: 'encryption', error });
    ...
    decrypt(encryptedData) {
        try {
            ...
            this.emit('error', { phase: 'decryption', error });
