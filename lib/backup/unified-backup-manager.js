...
export class UnifiedBackupManager extends EventEmitter {
  ...
  async createIncrementalBackup(backup) {
    ...
    const daysSinceFullBackup = this.state.lastFullBackup ? 
      (Date.now() - this.state.lastFullBackup) / (1000 * 60 * 60 * 24) : Infinity;
      
    if (!this.state.lastFullBackup || 
        daysSinceFullBackup >= this.config.incremental.fullBackupInterval ||
        this.state.incrementalCount >= this.config.incremental.maxIncrementals) {
    ...
    backup.metadata.incrementalNumber = ++this.state.incrementalCount;
    backup.metadata.basedOn = this.state.lastBackup;
    
    // For SQLite, we can use the backup API for incremental backups
    // This is a simplified version - real implementation would track page changes
    return this.createFullBackup(backup);
  }
