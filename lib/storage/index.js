...
export class ShareStore {
  ...
  async getMinerShares(minerId, since) {
    // Validate inputs
    const minerValidation = sqlValidator.validate(minerId, 'identifier');
    if (!minerValidation.valid) {
      throw new Error(`Invalid miner ID: ${minerValidation.error}`);
    }
    
    const sinceValidation = sqlValidator.validateNumeric(since, { min: 0 });
    if (!sinceValidation.valid) {
      throw new Error(`Invalid timestamp: ${sinceValidation.error}`);
    }
    
    const cutoff = Math.floor(parseInt(sinceValidation.value) / 1000);
    return this.storage.execute((db) => {
      return db.prepare(`
        SELECT * FROM shares
        WHERE miner_id = ? AND timestamp > ?
        ORDER BY timestamp DESC
      `).all(minerValidation.value, cutoff);
    });
  }
}
...
export class BlockStore {
  ...
  async getBlockByHeight(height) {
    // Validate height
    const validation = sqlValidator.validateNumeric(height, { min: 0 });
    if (!validation.valid) {
      throw new Error(`Invalid block height: ${validation.error}`);
    }
  ...
  async getBlockByHash(hash) {
    // Validate hash
    const validation = sqlValidator.validate(hash, 'hash');
    if (!validation.valid) {
      throw new Error(`Invalid block hash: ${validation.error}`);
    }
