...
export class StorageManager extends EventEmitter {
  ...
  async vacuum() {
    try {
      ...
      const deleted = this.db.prepare('DELETE FROM shares WHERE timestamp < ?').run(cutoff);
...
export class ShareStore {
  ...
  async initialize() {
    ...
    const shares = this.storage.statements.getRecentShares.all(cutoff, 10000);
  ...
  async addShare(share) {
    // Store in database
    const result = this.storage.execute(() => {
      return this.storage.statements.insertShare.run(
        share.minerId,
        share.jobId,
        share.nonce,
        share.difficulty,
...
export class BlockStore {
  ...
  async initialize() {
    // Load recent blocks
    const blocks = this.storage.db.prepare(`
      SELECT * FROM blocks
      WHERE is_orphan = 0
  ...
  async addBlock(block) {
    ...
    this.storage.execute(() => {
      this.storage.statements.insertBlock.run(
        block.height,
        block.hash,
        block.minerId,
        block.difficulty,
  ...
  async getBlockByHeight(height) {
    return this.storage.get(`block:${height}`, () => {
      return this.storage.db.prepare(
  ...
  async updateConfirmations(height, confirmations) {
    this.storage.execute(() => {
      this.storage.statements.updateBlockConfirmations.run(confirmations, height);
    });
...
export * from './blockchain/blockchain-integration.js';
