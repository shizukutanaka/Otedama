...
class WalletIntegrationService extends EventEmitter {
  constructor(options = {}) {
    super();
  ...
  async processMinerPayment(paymentId) {
    ...
    try {
      ...
      this.stats.totalVolume += this.convertToUSD(payment.amount, payment.currency);
  ...
  async stakeCurrency(minerId, currency, amount, duration) {
    ...
    this.stats.stakingVolume += this.convertToUSD(amount, currency);
