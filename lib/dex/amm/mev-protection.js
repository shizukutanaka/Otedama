...
export class MEVProtection extends EventEmitter {
    ...
    async executeBatch(batch) {
        ...
        for (const swap of batch) {
            try {
                ...
                const protectedParams = this.applyMEVProtection(swap.swapParams);
                
                // Check JIT liquidity
                if (this.isJITLiquidity(protectedParams)) {
    ...
    applyMEVProtection(params) {
        ...
        if (!protected.maxSlippage || protected.maxSlippage > this.config.maxSlippage) {
            protected.maxSlippage = this.config.maxSlippage;
        }
        
        // Add deadline if not present
        if (!protected.deadline) {
            protected.deadline = Date.now() + 300000; // 5 minutes
        }
        
        // Add recipient validation
        if (!protected.recipient) {
            protected.recipient = protected.from;
        }
