...
const logger = getLogger('MevProtection');
        ...
        for (const [pair, orders] of ordersByPair) {
            ...
            while (buyIndex < buyOrders.length && sellIndex < sellOrders.length) {
                ...
                if (buyPrice >= sellPrice) {
                    ...
                    // Clearing price with random component to prevent gaming
                    const randomFactor = crypto.randomBytes(1)[0] / 255; // 0-1 range
                    clearingPrice = sellPrice + (buyPrice - sellPrice) * randomFactor;
