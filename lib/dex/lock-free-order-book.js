...
class AtomicFreeList {
  ...
  take() {
    let index;
    let retries = 0;
    const maxRetries = 1000; // Prevent infinite spinning
    ...
    do {
      ...
      if (++retries > 10) {
        const backoffMs = Math.min(Math.pow(2, retries - 10), 100);
        const start = Date.now();
        while (Date.now() - start < backoffMs) {
          // Busy wait with reduced CPU usage
        }
      }
      
      if (retries >= maxRetries) {
        console.error('Lock-free take operation exceeded max retries');
        return -1;
      }
  ...
  add(index) {
    ...
    do {
      ...
      if (currentCount >= this.list.length) {
        console.error('Free list is full, cannot add index');
        return false;
      }
      ...
      if (retries >= maxRetries) {
        console.error('Lock-free add operation exceeded max retries');
        return false;
      }
    } while (!added && retries < maxRetries);
