  ...
  constructor(config = {}) {
    ...
    this.config = {
      ...
      exporters: config.exporters || {
        ...
        jaeger: {
          enabled: true,
          endpoint: process.env.JAEGER_ENDPOINT || `http://${process.env.JAEGER_HOST || 'localhost'}:${process.env.JAEGER_PORT || 14268}/api/traces`,
          serviceName: config.serviceName || 'otedama-pool'
        },
        zipkin: {
          enabled: false,
          endpoint: process.env.ZIPKIN_ENDPOINT || `http://${process.env.ZIPKIN_HOST || 'localhost'}:${process.env.ZIPKIN_PORT || 9411}/api/v2/spans`
        },
        otlp: {
          enabled: true,
          endpoint: process.env.OTLP_ENDPOINT || `http://${process.env.OTLP_HOST || 'localhost'}:${process.env.OTLP_PORT || 4318}/v1/traces`,
          headers: {}
        },
        datadog: {
          enabled: false,
