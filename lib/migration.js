...
export class MigrationManager {
   ...
   */
  async migrate() {
    console.log('[MIGRATE_DEBUG] Entering migrate()');
    await this.initialize();
    console.log('[MIGRATE_DEBUG] initialize() completed');
    
    console.log('Starting database migration...');
    
    try {
      // Acquire lock
      console.log('[MIGRATE_DEBUG] Acquiring lock...');
      await this.acquireLock();
      console.log('[MIGRATE_DEBUG] Lock acquired');
      
      // Get current version
      console.log('[MIGRATE_DEBUG] Getting current version...');
      const currentVersion = this.getCurrentVersion();
      console.log(`[MIGRATE_DEBUG] Current version: ${currentVersion || 'none'}`);
      console.log(`Current database version: ${currentVersion || 'none'}`);
      
      // Get pending migrations
      console.log('[MIGRATE_DEBUG] Getting pending migrations...');
      const pending = await this.getPendingMigrations(currentVersion);
      console.log(`[MIGRATE_DEBUG] Found ${pending.length} pending migrations`);
      ...
      for (const migration of pending) {
        console.log(`\n[MIGRATE_DEBUG] Applying migration: ${migration.version} - ${migration.name}`);
        
        const start = Date.now();
        
        try {
          console.log(`[MIGRATE_DEBUG] Starting transaction for ${migration.version}`);
          const runTransaction = this.db.transaction(() => {
            console.log(`[MIGRATE_DEBUG] Executing 'up' for ${migration.version}`);
            migration.up(this.db);
            console.log(`[MIGRATE_DEBUG] 'up' for ${migration.version} completed`);
            
            console.log(`[MIGRATE_DEBUG] Recording success for ${migration.version}`);
            this.db.prepare(`
            ...
            console.log(`[MIGRATE_DEBUG] Success recorded for ${migration.version}`);
          });

          runTransaction();
          
          migrated++;
          console.log(`[MIGRATE_DEBUG] Successfully applied migration: ${migration.version}`);
...
`;
  ...
  async loadMigration(version) {
    ...
    if (file.endsWith('.cjs')) {
      console.log(`[MIGRATE_DEBUG] Loading CommonJS migration: ${file}`);
      ...
      console.log(`[MIGRATE_DEBUG] Loading ES Module migration: ${file}`);
      try {
        const { pathToFileURL } = await import('url');
        const fileUrl = pathToFileURL(filepath).href;
        return await import(fileUrl);
      } catch (importError) {
        console.log(`[MIGRATE_DEBUG] Dynamic import failed: ${importError.message}`);
        throw importError;
      }
    }
  }
