...
export class CryptoUtils {
  ...
  randomInt(min = 0, max = Number.MAX_SAFE_INTEGER) {
    ...
    let randomValue;
    do {
      const bytes = cryptoRandomBytes(bytesNeeded);
  ...
  async hashPassword(password, salt = null) {
    ...
    try {
      const actualSalt = salt || cryptoRandomBytes(this.config.saltLength);
  ...
  async verifyPassword(password, storedHash, salt, algorithm = KeyDerivationFunction.SCRYPT) {
    try {
      ...
      return cryptoTimingSafeEqual(derivedHash, storedHashBuffer);
    } catch (error) {
  ...
  encrypt(plaintext, key, additionalData = null) {
    ...
    try {
      ...
      const iv = cryptoRandomBytes(this.config.ivLength);
