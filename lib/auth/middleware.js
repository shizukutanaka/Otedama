  ...
  return {
    ...
    requireAuth: async (req, res, next) => {
      try {
        ...
        // Check for API key with timing-safe verification
        const apiKey = req.headers['x-api-key'] || req.query.apiKey;
        if (apiKey) {
          try {
            // Always perform the same amount of work regardless of key validity
            const verificationPromise = authManager.verifyApiKey(apiKey);
            const minDelay = new Promise(resolve => setTimeout(resolve, 10)); // Minimum 10ms delay
            
            const [keyInfo] = await Promise.all([verificationPromise, minDelay]);
            ...
            if (keyInfo) {
              ...
              // Always take the same response path
              res.statusCode = 401;
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify({ error: 'Invalid API key' }));
            }
          } catch (error) {
            // Same error response for any exception
            res.statusCode = 401;
            res.setHeader('Content-Type', 'application/json');
            res.end(JSON.stringify({ error: 'Invalid API key' }));
          }
      ...
      return (req, res, next) => {
        ...
        if (!authManager.hasPermission(req.user.permissions, permission)) {
          ...
          res.end(JSON.stringify({ 
            ...
            // Removed permission exposure for security
          }));
          return;
        }
    ...
    optionalAuth: async (req, res, next) => {
      try {
        ...
        if (apiKey && !req.user) {
          try {
            const keyInfo = await authManager.verifyApiKey(apiKey);
            if (keyInfo) {
              req.user = {
                id: keyInfo.userId,
                permissions: keyInfo.permissions,
    ...
    /**
     * Rate limiting per user with memory leak protection
     */
    userRateLimit: (limit = 100, window = 60000) => {
      const requests = new Map();
      let lastCleanup = Date.now();
      const cleanupInterval = window * 2; // Cleanup every 2 windows
      ...
      const cleanup = (now) => {
        ...
        for (const [userId, userRequests] of requests) {
          const validRequests = userRequests.filter(time => time > cutoff);
          
          if (validRequests.length === 0) {
            // No recent requests, mark for deletion
        ...
        // Remove inactive users to prevent memory leak
        usersToDelete.forEach(userId => requests.delete(userId));
        lastCleanup = now;
      };
      ...
      return (req, res, next) => {
        ...
        // Clean old requests for current user
  ...
  return async (req, res) => {
    try {
      ...
      if (options.required !== false) {
        const authenticated = await middleware.requireAuth(req, res);
        if (!authenticated && res.headersSent) return;
      } else if (options.optional) {
        await middleware.optionalAuth(req, res);
      }
