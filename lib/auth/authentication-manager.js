...
export class AuthenticationManager extends EventEmitter {
  constructor(dbManager, options = {}) {
    ...
    // Initialize comprehensive validator and crypto utils
  ...
  async initializeDatabase() {
    try {
      ...
      // Create auth tables in parallel for faster initialization
      await Promise.all([
        this.db.query(`
      ...
      // Create indexes in parallel after tables are ready
      await Promise.all([
        this.db.query('CREATE INDEX IF NOT EXISTS idx_sessions_user ON user_sessions(user_id)'),
        this.db.query('CREATE INDEX IF NOT EXISTS idx_sessions_token ON user_sessions(token_hash)'),
        this.db.query('CREATE INDEX IF NOT EXISTS idx_apikeys_user ON api_keys(user_id)'),
        this.db.query('CREATE INDEX IF NOT EXISTS idx_apikeys_hash ON api_keys(key_hash)'),
        this.db.query('CREATE INDEX IF NOT EXISTS idx_login_ip ON login_attempts(ip_address, created_at)')
      ]);
  ...
  async register(userData) {
    ...
    try {
      ...
      const validationResult = this.validator.validateObject(userData, {
        ...CommonSchemas.user,
      ...
      if (validationResult.hasErrors()) {
        ...
        throw ErrorUtils.validation(`Validation failed: ${firstError.message}`, {
      ...
      if (existing) {
        throw ErrorUtils.validation('User already exists', {
      ...
      const apiKeyHash = this.crypto.hashApiKey(apiKey);
  ...
  async login(credentials, metadata = {}) {
    ...
    try {
      ...
      if (!user) {
        ...
        throw ErrorUtils.authentication('Invalid credentials', { username, ipAddress });
      }
      
      // Check account status
      if (user.status !== 'active') {
        throw ErrorUtils.authentication(`Account is ${user.status}`, { userId: user.id, status: user.status });
      }
      
      // Check lockout
      if (user.locked_until && new Date(user.locked_until) > new Date()) {
        throw ErrorUtils.authentication('Account is temporarily locked', { userId: user.id, lockedUntil: user.locked_until });
      }
      ...
      if (!validPassword) {
        ...
        throw ErrorUtils.authentication('Invalid credentials', { userId: user.id, ipAddress });
      }
      ...
      if (user.two_factor_enabled) {
        ...
        if (!valid2FA) {
          await this.recordLoginAttempt(username, ipAddress, false);
          throw ErrorUtils.authentication('Invalid 2FA token', { userId: user.id });
        }
      }
      ...
      // Parallelize login-related database operations for better performance
      ...
      const sessionData = this.encrypt(JSON.stringify({
        userId: user.id,
        role: user.role,
  ...
  async refreshAccessToken(refreshToken) {
    try {
      ...
      if (this.blacklist.has(refreshToken)) {
        throw ErrorUtils.authentication('Token has been revoked', { token: 'REDACTED' });
      }
      
      // Verify refresh token
      const payload = this.verifyToken(refreshToken, TokenType.REFRESH);
      if (!payload) {
        throw ErrorUtils.authentication('Invalid refresh token', { token: 'REDACTED' });
      }
      ...
      if (!session) {
        throw ErrorUtils.authentication('Session not found or expired', { sessionId });
      }
      ...
      if (!user || user.status !== 'active') {
        throw ErrorUtils.authentication('User not found or inactive', { userId: payload.userId });
      }
  ...
  async createApiKey(userId, name, permissions = null) {
    try {
      ...
      if (!user) {
        throw ErrorUtils.authentication('User not found', { userId });
      }
      
      // Generate API key
      const apiKey = this.crypto.generateApiKey();
      const keyHash = this.crypto.hashApiKey(apiKey);
  ...
  async verifyApiKey(apiKey) {
    try {
      ...
      // Check cache first (decrypt if found)
      const cachedEncrypted = this.apiKeys.get(keyHash);
      if (cachedEncrypted) {
        try {
          return JSON.parse(this.decrypt(cachedEncrypted));
        } catch (error) {
          // Cache corruption, remove and continue
      ...
      const encryptedResult = this.encrypt(JSON.stringify(result));
      this.apiKeys.set(keyHash, encryptedResult);
  ...
  async hashPassword(password) {
    const result = await this.crypto.hashPassword(password);
    ...
    return jwt.sign(
      {
        ...
        jti: this.crypto.randomBytes(16, 'hex')
      },
      this.config.jwtSecret,
  ...
  generateSessionId() {
    return this.crypto.generateSessionToken();
  }
  
  generateSecret() {
    return this.crypto.randomBytes(64, 'hex');
  }
  
  deriveEncryptionKey() {
    const secret = this.config.jwtSecret || this.generateSecret();
    return this.crypto.sha256(secret, 'buffer');
  }
  
  encrypt(text) {
    const result = this.crypto.encrypt(text, this.config.encryptionKey);
  ...
  decrypt(encrypted) {
    ...
    return this.crypto.decrypt(encryptedData, this.config.encryptionKey);
  }
  ...
  getSessionData(sessionId) {
    const encryptedData = this.sessions.get(sessionId);
    if (!encryptedData) return null;
    
    try {
      return JSON.parse(this.decrypt(encryptedData));
    } catch (error) {
      // Cache corruption, remove
      this.sessions.delete(sessionId);
      return null;
    }
  }
