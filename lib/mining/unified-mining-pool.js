...
import { StratumV2Handler } from './stratum-v2.js';
import { MergeMiningHandler } from './merge-mining.js';
import { ProxyHandler } from './proxy-handler.js';
import { DifficultyAdjuster } from './difficulty-adjuster.js';
import { NonceAllocator } from './nonce-allocator.js';
import { ShareValidator } from './share-validator.js';
import { VardiffManager } from './vardiff-manager.js';
import { BanManager } from './ban-manager.js';
import { JobManager } from './job-manager.js';
import { LoadBalancer } from './load-balancer.js';
import { HashratePrediction } from './hashrate-prediction.js';
import { HardwareValidator } from './hardware-validator.js';
import { ProfitSwitcher } from './profit-switcher.js';
import { DifficultyTracker } from './difficulty-tracker.js';
...
class UnifiedMiningPool extends EventEmitter {
    ...
    async start() {
        ...
        try {
            ...
            if (this.config.features.mergeMining) {
                await this.mergeMining.start();
                console.log('🔗 Merge mining enabled');
            }
    ...
    async shutdown() {
        ...
        // Shutdown feature modules
        if (this.stratumV2) await this.stratumV2.shutdown();
        if (this.proxy) await this.proxy.shutdown();
        if (this.profitSwitcher) this.profitSwitcher.stop();
        if (this.difficultyTracker) this.difficultyTracker.stop();
        if (this.mergeMining) await this.mergeMining.shutdown();
        
        this.emit('shutdown');
    }
}

export default UnifiedMiningPool;