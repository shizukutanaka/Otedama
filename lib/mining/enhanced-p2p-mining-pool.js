...
import { miningPools } from '../core/optimized-object-pool.js';
...
class ShareManager extends EventEmitter {
  ...
  /**
   * Add valid share with optimized processing
   */
  addShare(share) {
    // Pre-allocate timestamp for better performance
    const timestamp = Date.now();
    
    // Use circular buffer for better memory efficiency
    if (this.shares.length >= this.windowSize) {
      const removedShare = this.shares.shift();
      this.totalDifficulty -= BigInt(removedShare.difficulty);
      
      // Update miner stats for removed share
      const minerStats = this.sharesByMiner.get(removedShare.minerId);
      if (minerStats) {
        minerStats.shares--;
        minerStats.difficulty -= BigInt(removedShare.difficulty);
        if (minerStats.shares <= 0) {
          this.sharesByMiner.delete(removedShare.minerId);
        }
      }
    }
    ...
    // Update miner stats with optimized access
    let minerShares = this.sharesByMiner.get(share.minerId);
    if (!minerShares) {
      minerShares = {
        shares: 0,
        difficulty: 0n,
    ...
    minerShares.lastShare = timestamp;
    
    // Emit share event for monitoring
    ...
    // Window maintenance completed above
  }
  
  /**
   * Record invalid share with optimized object reuse
   */
  addInvalidShare(minerId, reason) {
    this.invalidShares++;
    
    // Use pooled object for event emission
    const eventData = miningPools.messagePool.acquire();
    eventData.type = 'share:rejected';
    eventData.data = { minerId, reason };
    eventData.timestamp = Date.now();
    
    this.emit('share:rejected', eventData.data);
    
    // Return object to pool
    miningPools.messagePool.release(eventData);
  }
