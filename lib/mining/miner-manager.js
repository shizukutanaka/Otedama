...
export class MinerManager extends EventEmitter {
  ...
  async detectNvidiaGPUs() {
    try {
      ...
      const result = await exec('nvidia-smi --query-gpu=name,memory.total,compute_cap --format=csv,noheader');
  ...
  async detectAMDGPUs() {
    try {
      ...
      // First try ROCm
      try {
        const result = await exec('rocm-smi --showproductname');
        // Parse ROCm output
        return { available: true, devices: [] }; // Simplified
      } catch {
        // Try OpenCL
        const result = await exec('clinfo -l');
