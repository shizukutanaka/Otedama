...
export class DistributedShareValidator extends EventEmitter {
  ...
  async verifyShare(share, options = {}) {
    ...
    try {
      ...
      const verifiers = this.selectVerifiers(share, options);
      
      if (verifiers.length < this.options.minVerifiers) {
        throw new Error(`Insufficient verifiers: ${verifiers.length}/${this.options.minVerifiers}`);
      }
      ...
      // Process responses
      const result = this.processVerificationResponses(shareId, responses, verifiers);
