...
export class EncryptedTransport extends EventEmitter {
    constructor(options = {}) {
        ...
        // Zero out keys on process exit
        process.on('exit', () => this.clearKeys());
        process.on('SIGINT', () => this.clearKeys());
        process.on('SIGTERM', () => this.clearKeys());
    ...
    constantTimeEqual(a, b) {
        ...
        bufA.writeUInt32LE(a, 0);
        bufB.writeUInt32LE(b, 0);
    ...
    clearKeys() {
        // Clear session keys
        for (const [id, keys] of this.sessionKeys) {
            if (keys.sendingKey) crypto.randomFillSync(keys.sendingKey);
            if (keys.receivingKey) crypto.randomFillSync(keys.receivingKey);
        }
        this.sessionKeys.clear();
        
        // Clear ephemeral keys
        for (const [id, keyPair] of this.ephemeralKeys) {
            if (keyPair.privateKey) {
                const privateKeyData = keyPair.privateKey.export({ type: 'pkcs8', format: 'der' });
                crypto.randomFillSync(privateKeyData);
            }
        }
        this.ephemeralKeys.clear();
        
        // Note: Static keys should be handled carefully as they may be needed for restart
    }
    
    /**
     * Update connection statistics safely
     */
    updateConnectionStats(connectionId, direction, bytes) {
        const connection = this.activeConnections.get(connectionId);
        ...
        if (direction === 'in') {
            connection.bytesIn = (connection.bytesIn || 0) + bytes;
        } else {
            connection.bytesOut = (connection.bytesOut || 0) + bytes;
        }
        connection.lastActivity = Date.now();
    }
}
