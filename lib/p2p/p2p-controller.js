...
export class P2PController extends EventEmitter {
  constructor(options = {}) {
    ...
    // State management with limits and LRU cache
    this.peers = new Map();
    this.miners = new Map();
    
    // Use LRU cache for shares and blocks to limit memory usage
    const LRU = require('lru-cache');
    this.shares = new LRU({
      max: this.options.maxShares || 10000,
      ttl: this.options.shareRetentionTime || 3600000, // 1 hour
      allowStale: false,
    ...
    this.blocks = new LRU({
      max: this.options.maxBlocks || 1000,
      ttl: this.options.blockRetentionTime || 86400000, // 24 hours
      allowStale: false
    });
    
    // Security features with automatic cleanup
    this.messageRateLimits = new LRU({
      max: 10000, // Limit tracked peers
    ...
    // Removed - now handled by LRU cache configuration
  ...
  async initialize() {
    try {
      ...
      const initPromises = [
        ...
        this.meshTopology.initialize(),
        this.gossipProtocol.initialize(),
        this.stratumV2.start()
      ];
