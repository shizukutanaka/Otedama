...
```
Otedama/
├── cmd/otedama/            # CLI エントリポイント（run/version/config/service/doctor/arb/completion）
├── internal/
...
秘密情報（APIキー、秘密鍵、パスワード、シード）のコミットは絶対禁止です。`.gitignore` に適切なパターンを含め、`.pre-commit-config.yaml` のフック（`pre-commit install` で有効化、gitleaks + detect-private-key）で検出します。環境変数またはHashiCorp Vaultなどの専用シークレット管理システムを使用します。
