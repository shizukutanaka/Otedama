...
**ブランチの復元・master との統合・レビュー可能化（2026-09-24）**

コンテナが再作成され、ローカルのブランチが **`master` から作り直されていた**——このクローンには
本ブランチの12コミットのオブジェクトすら無かった。リモートは無事（`2f9b232`）、ローカルに固有の
コミットは0件だったので、リモートからの復元で何も失っていない。ただし気づかずに commit/push していれば
乖離させるか上書きしていた。

さらに**このブランチには PR が一度も作られていなかった**。リポジトリには PR が333件あり、マージ済みは
5件だけ、並行エージェント（`devin/…`、`feature/session-2xx-cs`）の未マージ PR が約328件。
本ブランチの既定プール撤廃・`go 1.24` 化・ランタイムメトリクス配線・到達不能コード検出・
約40件の文書訂正は、**メンテナから一切見えていなかった**。今回ドラフト PR を作成する。
（なお、並行エージェントも「session 264/265/266」という番号を使っている。本ブランチの
session 番号はこのブランチの系譜だけを指し、`feature/session-2xx-cs` とは無関係。）

`master` の PR #5（Actions v3→v4）・#6（`dependabot.yml` に `automerge:`）を統合:
- 本ブランチが削除済みの `ci-cd.yml`・`deploy.yml` で modify/delete 競合 → **削除を維持**
  （PR #5 の変更は中のバージョン番号だけで、両ファイルが死んでいる理由——存在しない
  `package.json`・helm chart・k8s ディレクトリ——は変わらない。§13）。
- **予測が外れた**: master の workflow 変更を含むマージの push は、App の `workflows` 権限不足で
  拒否されると予測していたが、**受理された**。境界は「ブランチ上の workflow を変えられない」
  ではなく「**リポジトリに未存在の workflow 内容を書けない**」だった（§13 に追記）。

`master` 側の主張を、断定ではなく測定で確認:
- PR #5 の「drop-in-safe」: `upload-artifact@v4` は matrix の各レグで同名アーティファクトを
  拒否するが、該当ステップは全て matrix 由来の名前か1レグに限定されていた。**主張は成立**、
  私の疑いは誤り。docker/*@v4 の存在と codecov v4 の入力互換は egress 制限で未確認（調査が必要）。
- **Dependabot は一度も PR を開いていない**（`author:app/dependabot` 検索で0件）。
  AUDIT_CHECKLIST で私自身が「PASS」としていた行は、**ファイルがあることを以て稼働とみなしていた**——
  本セッションが他の文書で繰り返し正してきた「設定≠稼働」の誤りを自分の行でしていた。
  AUDIT_CHECKLIST・§21・THREAT_MODEL・SUSTAINABILITY・solo-operations の5箇所を訂正。
  PR #6 の `automerge:` キーが Dependabot のスキーマに有効かは docs.github.com が egress ブロックのため
  確認できず「調査が必要」とした（無効なら全エコシステムの更新が止まるが、0件は PR #6 以前からなので
  どちらの証拠にもならない）。

**長所・短所の再棚卸し（session 265 の表を差し替え）**

session 265 も長所・短所を表にしていた。**その表は差し替えが必要だった**——根拠の一部が、
本セッションで偽と判明した文書だったからである（「10言語対応」「SHA ピン留め済み」
「cosign 署名済み」「Noise NX がプールを認証する」）。**誤った文書から長所を数えると、
長所の数だけ嘘が増える。** `docs/CATEGORY_AUDIT.md` 末尾に、**各行に測定値・テスト名・§番号を
付けた**新しい表を追加し、旧表には差し替え注記を付けた。

本日実測して長所欄に入れた数字: 単スレッド **5.56 MH/s** / 4スレッド **23.5 MH/s**
（`BenchmarkWorkerGrind_*`）、test:impl = **36,829:20,660 = 1.78**、最低カバレッジ **91.4%**
（7パッケージが100%）、リンクされる外部モジュール **3**、KNOWN_LIMITATIONS **23項目中14解決**。

短所欄の筆頭は変わらず **`stratum+v2://` の平文**（§2）。加えて本セッションの変更が生んだ代償
——**プール設定が必須になった**（§20 の既定撤廃）——も短所として明記した。
既定が死んでいた事実の開示と、その修正がユーザー体験に課したコストは、別々に書く必要がある。

**`skills/` — 自分の監査主張が範囲外にしていた面を歩く**

本セッションの完成文は「`docs/` の全 `.md` と `README.md`・`install.sh`」と範囲を限定していた。
その限定は正確で、だからこそ示唆的だった——`skills/`（6ファイル559行、リリース・レビュー・監査・TDD の
**実行手順書**）は一度も読んでいない。主張を真にするために引いた境界線は、越える価値がある。

`release-procedure.md` は**リポジトリ内で最も危険な文書**だった。ゲートだからである。
実行不能な手順でリリースを止め、存在しない自動化を前提にしていた:

- 「v2移行テスト `otedama migrate-from-v2` が成功すること」→ **そのサブコマンドは存在しない**
- 「各バイナリに SHA-256 チェックサム・GPG署名・SBOM を含める」→ **3つとも生成されない**（§21）
- 「Docker Hub へのイメージ公開も自動化」→ 実際は ghcr.io
- 「Homebrew / Snap / AUR への展開は自動化されています」→ Snap・AUR は**0件**、
  Homebrew ジョブは他リポジトリを `GITHUB_TOKEN` で checkout しようとしており原理的に動かない
- 「FreeBSD のビルドとスモークテスト」→ リリース対象外
- 「公式Discordサーバーの #announcements に投稿」→ `solo-operations.md` は Discord を
  「ソロメンテナのバーンアウトを最速で引き起こす」として**明示的に不採用**。2文書が正反対を指示
- 「クラッシュレポート・テレメトリを確認」→ **一切 phone home しない設計**（SUSTAINABILITY §7）
- 「主要10言語のドキュメントが更新されていること」→ ドキュメントは日英のみ

他3件: `security-audit.md` の「`govulncheck` を CI で毎回実行」「CI でファズを継続実行」（**両方存在しない**）、
`code-review.md` の「CODEOWNERS により独立した二人以上の二重レビュー」（**実際は所有者1名**。
bus-factor 1 は他文書が認めている）、`tdd.md` の「CI がカバレッジ低下を警告」
（闘値はリポジトリに無く、アップロード段を含む `ci.yml` は現在そもそも成功しない——§13）。

全て「著者が意図したシステムを現在形で書き、実在するシステムと突き合わせないまま時間が経った」
という本セッション共通のパターン。**手順書は最も危険な置き場所**である——再導出せずに従われることが
その存在意義だからである。

**`go 1.22` → `go 1.24`（改善点 #5）— 「メンテナの判断」も、証拠を集めれば判断できる**

これも「14個の実行時デフォルトが変わるのでメンテナの判断」と分類していた。だが
**その14個を一度も列挙していなかった**。ツールチェーン同梱の
`src/internal/godebugs/table.go` を読めば列挙できる。読んだ。

- **`go.mod` を `go 1.24` にし、`godebug` ブロックを全削除した。**
  測定: `go 1.22` の頃のバイナリは `DefaultGODEBUG` に**17項目**を焼き込んでいた
  （`asynctimerchan=1`, `multipathtcp=0`, `tls3des=1`, `x509negativeserial=1`, … ）。
  `go 1.24` + ピン無しでは **`DefaultGODEBUG=` が空**——全設定がツールチェーンの現行既定、
  すなわち `tlsmlkem=1`（PQ 鍵交換有効）も含めて既定で有効になる。
  値が実際に変わるのは14個で、変わらない3個は**まさに従来ピンしていた3個**だった
  （ピンが「後に既定になる値」を先取りして押さえていたため）。
- **`tlsmlkem=1` の罠が消えた。** `go 1.22` 下ではこの1行の削除が
  ポスト量子鍵交換を無言で無効化した（session 264 に測定済み）。`go 1.24` では既定なので、
  ピン自体が不要になり、「消してはいけない1行」という危険な構造がなくなった。
- **§13 最後の推測が出典に置き換わった。** Go 同梱の `doc/godebug.md` が
  *"Go 1.24 also removed X25519Kyber768Draft00 and the Go 1.23 `tlskyber` setting."* と明記。
  1.24 の godebug テーブルに `tlskyber` は存在せず `tlsmlkem` は `Changed: 24`。
  よって「Go 1.23 は `tlsmlkem` を知らない → 未知キーで `go.mod` ロード時エラー」は
  推測ではなく出典に基づく事実になった。
- **安全性の根拠（推測ではなく実行）:** 変化する14個のうち6個は暗号のより厳格な既定
  （資金を扱うバイナリには望ましい方向）、7個は本コードベースに無関係
  （`rand.Seed`・`ServeContent`・`os.Symlink`・MPTCP・3DES・`crypto/rsa` の直接使用は**全て0件**、grep で確認）。
  実害があり得るのは **`asynctimerchan` 1→0**（Go 1.23 のタイマーチャネル意味論変更）で、
  本コードベースには timer/ticker が25箇所ある。**全スイート3回連続 green（24/24）、
  `-race` も 24/24 データレース無し、タイマー最密の `internal/engine` を5回連続 green、
  `miner`/`httpserver`/`rates`/`tui` を各3回 green。** 実バイナリの挙動も不変
  （プール無し=78、プール有り=0）。
- `GODEBUG_NOTES.md` を全面改訂（baseline・Active knobs・何が動いたか・どう確かめたか）。
  同ファイル自身の「`go` 行は該当 Go リリースの6ヶ月以上後に上げる」規定にも適合
  （Go 1.24.0 は2025年2月、採用は2026年9月＝19ヶ月後）。
  ただし規定の `staticcheck` 実行は**未実施**（未インストール、CI にも無い——§21）と明記した。
...
`cmd/otedama/unreachable_test.go` は `internal/` と `cmd/` の全 Go ファイルを AST で解析し、
**テストからしか参照されていないエクスポート済み関数・メソッド**を列挙する。現在の33件を
ベースラインとして固定し、**増えたら失敗**する。加えて、実体を失ったベースライン項目
（古い免除）も失敗させる — 免除が対象より長生きすると次の事例を隠すため。

実際に効くことの確認（両方実行）:

- 本セッションで足した `RegisterCollector` の1行を外すと、0.12秒で
  `metrics.RuntimeCollector is exported, exercised by tests, and called by no non-test code` と
  して落ちる。**数時間前に手で見つけた欠陥を機械が再現した。**
- 存在しない関数をベースラインに足すと
  `knownTestOnlyFuncs lists "clock.NoSuchFunction", which no longer exists` で落ちる。

ベースラインは欠陥一覧ではない。**意図的なテスト継ぎ目**（`clock.NewFake` 等）と
**取り残された機能**（`internal/stratum/noise.go` のエクスポート済みシンボル7件 = §2 の機械的表現）を
理由つきで区別している。手書きスキャンが見落としていた `HandshakeState.Transport` と
`EncryptedConn.Read` もメソッドとして検出した。
限界も明記した: インタフェース充足・リフレクション・「死んだ関数からのみ呼ばれる関数」は見えない。
煙感知器であって証明ではない。

**同じスキャンから出たもう1件の実欠陥**: `rates.Fetcher.SetLogger`（全ソース失敗・異常値・
TLS 検証や nTime を無効化しうる時計ずれの通知）**を誰も設定していなかった**ため、
全メッセージが nil 関数に送られていた。フェッチャは黙ってハードコードの $95,000 フォールバックを
返し続けるので、ダッシュボードの無い運用者には**もっともらしい価格が出続けるだけ**で、
それが何時間古いかは分からない。`curtail_below_btc_usd`（採掘を止めるか否か）はこの数値で判断する。
エンジン側に `newRateFetcher` を置き、**ログを付けてからでないとフェッチャを取得できない**構造にした
（テストは、取り消し済みコンテキストで初回フェッチを確実に失敗させ、warn ログが届くことを検査。
配線を外すと落ちることを確認済み）。
...
「その根拠は何か」を問う**——を重ね、どのセッションも検分していなかった最後の領域を掃いた。

**掃き出しの結果は「反証」ではなく「根拠の確認」だった.**

- **i18n は主張を稼いでいる**: 10言語×15メッセージIDの実翻訳（機械的コピーではない）に、
  完全性・placeholder一致・テンプレート解析・空文字禁止・10言語数を強制するテスト群
  （`messages_test.go`）。ただし消費面はエンジン起動時の logln のみで、TUI・doctor・wallet は
  英語のみ——この**範囲**をパッケージdocに明記した。テストが確立できるのは機械検証可能な性質までで、
  「人間レビュー済み」はその外にあることも正確に書いた（テストがそれを主張しないように）。
- **arbitration は CLAUDE.md のプロパティテスト要求を満たす**（配分不変条件のプロパティテスト5本）。
- **未監査ドキュメント4本は陳腐化参照ゼロ**（削除済みシミュレーション市場・80カラム・wallet不在への
  言及なし）。

**成果物**: `docs/CATEGORY_AUDIT.md` の session 265 エントリ——長所（各行に証拠）、
短所（各行に KNOWN_LIMITATIONS §番号とブロッカー）、改善点（優先順・工数・誰の判断か）、
そして完成宣言。

**完成の定義**（session 264 から一貫）: 「製品が主張することを正確に行い、行わないことは
理由・根拠・目標つきで開示されている」。20件の制約のうち13解決＋1部分解決、未解決の全件が
ブロッカーを名指しし、そのどれも「誰も見ていない」ではない。最後の未検分領域への問答が
確認で終わった——**それ以上問うても答えが変わらなくなった点**——をもって、この定義での完成を
宣言する。反証条件も書いた: 読者が証拠まで辿れない肯定的記述が repo に1つでもあれば、
この宣言は破れる。それが今後の全変更に対する常設のテストである。


### Fixed / Disclosed (session 264 — 自分が追加した拡張点にドリフトガードを入れ、既定プールの未検証を開示)

**自分の変更が同じ罠を作っていた.** 前項で追加した `doctor.schemeIsEncrypted` は、
Issue #3 が記録するスキーム知識の重複（`poolproto.knownSchemes` が正典、
`config.validatePoolURL`、`doctor.stripScheme`）に**4つ目を足すもの**だった。
記録されているリスクは「poolproto にスキームが追加されると doctor が黙ってズレる」。

幸い、私の `default` 分岐は `false`（＝暗号化されていない扱い）を返す。**フェイルセーフの向きは
正しい**——未知のスキームは「安心」ではなく「警告」を生む。誤って暗号化済みと分類すれば
ユーザーは支払いアドレスが守られていると信じ、誤って平文と分類しても偽の警告が出るだけである。
この非対称性こそがこの関数の存在理由なので、テストで固定した（`default` を `true` に
反転させると2件失敗することを確認）。

併せて poolproto とのドリフトガードを追加。**捕捉できるのはリネーム・削除**で、
**追加は捕捉できない**（`knownSchemes` は非公開で列挙できない）ことをテストのコメントに
明記した——できないことをできると書かないため。追加方向はフェイルセーフで守られている。

**既定プールについて調べ、確認できなかったことを開示（新規 §20）.**
`config.DefaultPoolURL` = `stratum+v2://public.stratum.slushpool.com:3336` は
session 78 で4箇所のコピペから単一ソース化されたが、**単一ソース化は検証ではない**。
このホストが解決するか、このポートが応答するかを確認したセッションは存在せず、
本セッションも外向きDNS/TCPが無いため確認できない。

これは通常の陳腐化した定数より重い。**設定ゼロのユーザー全員が到達する終点**であり、
README が先頭に置く `otedama run --bitcoin-address bc1q...` の経路そのものだからである。
そして `CLAUDE.md` は存在しないURLの記載を禁じているので、「未検証」は暗黙にせず明示すべき状態である。

**解決可否とは独立に、既定は平文である**——`stratum+v2://` は転送暗号化を得ない（§2）。
つまり**既定構成は、製品自身の診断が警告する構成**になった（前項の修正の結果）。
これは文書だけの問題ではなく設計判断で、選択肢は「既定をTLS終点にする」か
「設定ゼロ経路で明示的なプール選択を求める」かのいずれか。検証できないままTLSのホストとポートを
選ぶことは**終点をでっち上げること**であり、まさに上記の規則が禁じる行為なので、
メンテナ判断として記録するに留めた。

**判定方法も書いた**: ネットワークのある実機で `otedama doctor` を1回走らせれば、
Pool reachability チェックが答えを出す。だからこれは修正ではなく開示である。
...
**同じパッケージで見つけた3件目——検証されていない肯定的主張.** `doctor` の全 Pass メッセージを
洗ったところ、データディレクトリのチェックが **"(exists, writable)"** と報告しながら、
実際には `os.Stat` と world/group パーミッションビットしか見ておらず、**書き込み可能性を
一度も試していなかった**。所有者・ACL・read-only マウント・immutable フラグのいずれも、
モードビットは正常に見えたまま書き込みを拒否しうる。このディレクトリには `wallet.dat` が
置かれるので、誤答はウォレット書き込み時——まさに `doctor` が先回りするために存在する瞬間——に
露見する。一時ファイルを作って即削除する実書き込みプローブに置き換えた（診断は探ってよいが、
散らかしてはならないので、プローブファイルが残らないこともテストで固定）。

root 環境ではモードビットが書き込みを制限しないため、書き込み不可テストは skip する
（PTY テストと同じ、環境を偽らない設計）。**非rootユーザーで実際に実行して PASS を確認済み**
——CI と開発機はそちらで走る。

`internal/doctor` のカバレッジ 99.4%、`internal/poolproto/stratumv2` 97.0%。
...
**プロファイリング結果に裏付けられる場合にのみ適用**し、推測に基づく早期最適化を禁じている。
まずベースラインを取り、候補を個別にベンチマークし、勝った2つだけを実装した。

**候補の測定**（Xeon @2.10GHz、SHA-NI 無し、2秒ベンチ、全て 0 allocs）:

| 実装 | ns/op | 対現行 |
|---|---:|---:|
| 現行 `HashHeader`（毎回80バイト再構築＋3圧縮） | 264.2 | — |
| ヘッダを1度だけ直列化し nonce 4バイトのみ書き換え | 224.0 | −15% |
| **midstate**（先頭64バイトの圧縮結果を再利用） | 167.7 | **−36%** |

ブロックヘッダは80バイト＝SHA-256 の2ブロック。nonce が変わるのは**第2ブロックだけ**なので、
第1圧縮（version・prevhash・merkle root の先頭28バイト）は全 nonce で同一である。
ジョブごとに1回計算すれば、3圧縮のうち1つが消える——全ビットコインマイナーが使う標準技法。

**自前暗号ではない.** SHA-256 の内部は一切再実装していない。保存する状態は `crypto/sha256`
自身の `encoding.BinaryMarshaler` から得て `BinaryUnmarshaler` で復元するので、圧縮関数・
パディング・長さ計上は全て標準ライブラリのものである。`CLAUDE.md` の自前暗号禁止を構造的に守り、
`HashHeader` は平明な参照実装として残したうえで、等価性をテストで固定した。

**第2の発見: ハッシュごとのアトミック加算.** `w.hashCount.Add(1)` はハッシュごとに全スレッドが
同一キャッシュラインを奪い合っていた。4スレッド下で**30.5 ns/op**（バッチ化なら 0.15 ns/op）。
1024 nonce のバッチごとに1回の加算へ変更した。

**実ワーカーを通した実測（マイクロベンチではなく end-to-end）:**

| 測定 | 変更前 | 変更後 | 比 |
|---|---:|---:|---:|
| 1スレッド | 3.73 MH/s | 5.45 MH/s | **1.46倍** |
| 4スレッド | 9.64 MH/s | 21.9 MH/s | **2.28倍** |
| 1ハッシュあたり（4スレッド時、スレッド毎） | 415 ns | 182 ns | −56% |

マルチスレッドの利得（2.28倍）がシングル（1.46倍）を上回るのは、アトミック競合がスレッド数に
比例して効いていたからである。変更後は 4スレッド時のスレッド毎コスト 182 ns が
シングルスレッドの 183 ns とほぼ一致する——**競合が消えて線形スケールになった**ことを意味する。

**ベンチマーク自体の欠陥も是正.** `BenchmarkWorkerGrind_SingleThread` は
「性能リグレッション検出のベースライン」を名乗りながら、`grind` を**一度も呼ばず**
自前のループで `HashHeader` を回していた。だから 33% の変更に対して**数値が全く動かなかった**。
実 `Worker` を駆動して、ワーカー自身が持つカウンタから ns/hash を導く形に置き換え、
製品が実際に使うスレッド数での計測（`BenchmarkWorkerGrind_AllThreads`）も追加した。

**バッチ化による観測可能な意味変化を2つのテストに反映.** ハッシュカウンタはバッチ境界
（実測 ~180µs 相当）でのみ可視になるため、「最初のシェアと同時にカウンタが非ゼロ」を
仮定していた `TestWorker_StatsAfterWork` と、hashrate ゲージを「stats-tick が走った」ことの
代理として即時参照していた `TestRunSession_StatsTickAndShareResponses` を、
瞬間の一致ではなく**不変条件をポーリングする**形に修正した。

**空振り確認.** midstate の分割境界を1バイトずらすと、等価性テスト3件が全て失敗する。

**`BENCHMARKS.md` の捏造データを削除.** 単一/全コアのハッシュレート表は Ryzen 9 7950X・
Apple M2 Pro・i7-12700K・Raspberry Pi 5 の数値を載せていたが、**どれも測定されていない**。
「全コア」表の再現コマンドは `-cpu=1,2,4,8,16` を**シングルスレッドのベンチマーク**に
渡すもので、その表を生成しえない。同ファイル自身の測定哲学（「同じハードウェアを持つ誰もが
検証できること」）に反するため、実測した1台のみに置き換えた。併せて
「>5%のリグレッションで自動的に失敗する」「CIがPRに比較を投稿する」という記述も是正した
——CI の `benchmark` ジョブは実行して成果物をアップロードするだけで、比較もゲートもしていない。

### Added (session 264 — §16の残り半分を解決: **実装・テスト済みなのに到達不能だった `ChangePassphrase` を配線**)

**問題.** `lightning.WalletManager.ChangePassphrase` は旧パスフレーズをラウンドトリップ復号で
検証し、作成時と同じアトミック経路（同一ディレクトリの一時ファイル→書き込み→Sync→
Close（エラー検査あり）→chmod 0600→rename）で書き込み、テストも通っていた。
**しかし本番コードからの呼び出しが1箇所も無かった**ため、パスフレーズが漏洩した可能性のある
ユーザーは、internal パッケージに対して自前のGoプログラムを書かない限りローテーションできなかった。

**追加した薄いラッパ.** 暗号処理もファイル処理も新規には持たない。追加したのは、
ライブラリ関数には持たせられない運用者向けの安全性である:

- ウォレットが存在しない場合は `NewWalletManager` の新規作成経路に入る前に停止する
  （検証対象を自分で作り出しては意味がないのと同じ理由）。
- 新パスフレーズを**2回**読み、一致を定数時間比較で確認する。ここでのタイプミスは
  ユーザーを自分のウォレットから締め出すからである。
- **フィンガープリント検査が本題であり装飾ではない.** ローテーションは**同じシード**を
  再暗号化しなければならない。もしフィンガープリントが変われば、ユーザーが書き留めた
  リカバリフレーズはディスク上のウォレットを記述しなくなる——黙って、そして復旧試行時にのみ
  露見する形で。書き換え後に**新パスフレーズで開き直して**フィンガープリントを比較し、
  相違を成功ではなく失敗として扱う。到達不能なはずだが、ここで誤る代償はユーザーの資金である。

**`internal/lightning` は無変更**（verifyと同じ理由——必要な部品は全てエクスポート済み）。

**実バイナリで往復検証.** `otedama run` で作成 → ローテーション → 検証:
フィンガープリントは不変（`6c993fa2`）、**旧パスフレーズではウォレットが開かなくなり**、
元の24語リカバリフレーズは新パスフレーズ下で MATCH、`otedama doctor` も同じ
フィンガープリントを報告。

**テストが実バグを1件検出.** `readLine` は当初、呼び出しごとに新しい `bufio.Scanner` を
生成していた。Scannerは先読みバッファを持つため、**1回目の読み取りが確認用の2行目を
飲み込み**、ユーザーが正しく入力し終えた後に「no input was given」で失敗していた。
両方の読み取りで1つのScannerを共有するよう修正し、2回目の読み取りが2行目を見ることを
テストで固定した。

**カバレッジ.** `cmd/otedama` は新規コード追加で一時 88.7% に落ちた（プロジェクト下限90%を
下回る）。到達可能な未カバー経路——サブコマンドのディスパッチ、不正フラグ、データディレクトリを
決定できない環境、環境変数未設定、各プロンプトでのEOF、stdinのI/Oエラー——にテストを追加して
92.0%。防御的にのみ存在する分岐（フィンガープリント相違、再オープン失敗）は**意図的に
未カバーのまま**とした。`CLAUDE.md` はカバレッジの水増しを禁じている。
...
`"mnemonic" + パスフレーズのUTF-8 NFKD形` と規定する。`MnemonicToSeed` は PBKDF2 自体は
仕様どおりだが**正規化しない**。入力された非ASCIIパスフレーズはほぼ常にNFC
（`é` = U+00E9、`パ` = U+30D1）で、NFKDはこれを分解する。したがってOtedamaは、同じ
リカバリフレーズと同じパスフレーズから他のあらゆる準拠ウォレットが導出するのとは**別のシードを
導出**していた。表示されたフレーズは、Electrumやハードウェアウォレットで
**別の・一見正常な**ウォレットを黙って復元する。相手側はそれを検出できない——誤ったシードを
導出したことを知る術が原理的に無いからである。

**決定: 正規化ではなく拒否.** 2案がメンテナ判断待ちとして記録されていた。
`golang.org/x/text/unicode/norm` による正規化はBIP-39実装が本来「そうすべき」ことだが、
ADR-003（ランタイム依存ゼロ）が防ごうとしているランタイム依存と引き換えになる。代替の
NFKD自前実装は、`CLAUDE.md` が資金クリティカル領域で禁じる「標準アルゴリズムの自前実装」
そのものである。**正しく扱えない入力を受け付けるという要件の方を落とす**のが、
迂回すべき制約ではなく削るべき要件だった。

**挙動.** 非ASCIIのBIP-39パスフレーズでのウォレット作成は
`lightning.ErrNonPortablePassphrase` で失敗し、**問題の文字とそのバイト位置を名指しする**
——典型的な原因はWebページから貼り付けた不可視の1文字（ノーブレークスペース）だからである。
`otedama run` は設定時に同じ検査を行い exit 78 で終了する。`engine.setupWallet` は
ウォレット失敗を warn で記録して**ウォレット無しで続行**し、TUI有効かつ `--log-file` 無しでは
ロガーが破棄シンクなので、ライブラリ側の拒否だけに頼ると**理由の分からない
「ウォレットができない実行」**になってしまう。

**誰も締め出さない.** 検査は**作成時のみ**。`loadExisting` はニーモニックパスフレーズを
一切参照しない（保存されるのはシードのみで再導出しない）ので、非ASCIIパスフレーズで
作成済みのウォレットは通常どおり開き、`otedama wallet verify` は与えられたパスフレーズで
導出するので既存ウォレットの検証もできる。両方をテストで固定。

**ASCIIはNFKDの不動点**なので、ASCIIパスフレーズ（既定の空文字列を含む——これが通常のケース）は
完全に準拠している。この検査は「Otedamaが正しく導出できる入力」とちょうど一致する。

**テストの落とし穴を1つ潰した.** `TestRun_NonASCIIMnemonicPassphraseIsAConfigError` は当初
`--dry-run` 無しで書いたため、**検査を外すとテストがエンジンを起動してハングした**
（空振り確認の実行がタイムアウトして発覚）。リグレッションはミリ秒で失敗すべきで、
スイートを固まらせるべきではない。検査は dry-run の早期リターンより前にあるため、
`--dry-run` を付けても正しい実装は拒否し、検査が無ければ即座に exitOK になる。修正後、
空振り確認は4msで失敗することを確認。

### Fixed (session 264 — ③簡素化の対象を実害から選ぶ: **TUIが端末幅を検出せず80桁固定だったため、80桁未満の端末でダッシュボードが崩壊していた**)

**実害の性質.** 単に窮屈だっただけではない。再描画はカーソルをホームに戻して固定行数を
上書きする方式なので、端末が想定幅より狭いと**全行が折り返し**、折り返しごとに画面行を1つ
余分に消費し、以降の行のオフセットが合わなくなる——ダッシュボードは重なり合った断片に崩壊する。
`SetWidth` という注入点は存在したが、**テスト以外のどこからも呼ばれていなかった**。

**是正.** Linuxでは書き込み先のファイルディスクリプタから `TIOCGWINSZ` ioctl で実幅を読み
（`internal/tui/width_linux.go`）、描画ティックごとに読み直す。これによりセッション中の
リサイズに1秒以内で追従する——`SIGWINCH` ハンドラを持つよりも部品が少なく、ハンドラ設置中に
届いたリサイズを取りこぼすこともない。40桁（レイアウトの設計下限）未満の報告値はクランプする。

**依存を足さずに実現した根拠.** `golang.org/x/term` は同じことをするが、`CLAUDE.md` の
依存追加基準の第一項は「標準ライブラリで代替不可能であること」。TIOCGWINSZ は定数2つと
4フィールドの構造体であり、標準ライブラリで足りる。代償はリポジトリ唯一の `unsafe.Pointer` 1箇所
（x/sys 自身を含む、あらゆる Go の ioctl ラッパーと同じ書き方）——依存ツリーに新しい
モジュールを増やすより小さく、検査しやすい面である。

**Linux限定である理由を明示.** BSD系（macOS含む）は TIOCGWINSZ の値が異なり、
`syscall.Syscall` ではなく libc トランポリン経由で ioctl に到達する。Windowsは
`GetConsoleScreenBufferInfo` が必要。**実行して確かめられない環境向けに書くのは推測**なので、
他プラットフォームはスタブが「不明」を返し、ダッシュボードは従来どおりの既定幅で動く
（GPU検出が Linux 限定であるのと同じ方針 — §4）。

**テストは実PTYを使う.** `/dev/ptmx` を開いて `TIOCSWINSZ` でサイズを設定し、
132桁・120桁・100桁→160桁のリサイズ・20桁のクランプ・52桁で全行が折り返さないことを検証。
スタブでは検査できないもの——ioctl そのもの、構造体レイアウト、`ws_col` のオフセット——が
まさにここで壊れうる箇所である（オフセットを誤れば**行数を幅として黙って報告**する）。
検出を外すと4件中3件が失敗することを確認済み。

**併せて解消した潜在的データ競合.** `cols` は `SetWidth`（任意のゴルーチン）が書き、
描画ループが読む plain `int` だった。`SetWidth` がテスト以外から呼ばれていなかったため
表面化していなかっただけである。`atomic.Int32` にした。`-race` 通過。

`internal/tui` のカバレッジ 98.6%。

### Added (session 264 — マスクのアルゴリズム③④の前に、製品の中核的約束が**実行不能**だった点を是正: **`otedama wallet verify`**——バックアップが正しいか確認する手段が存在しなかった)

**要件を疑う（①）.** Otedamaは非カストディを中核的約束として掲げる。その約束は
「あなたが鍵を持つ」——具体的には、初回起動時に一度だけ表示されるBIP-39リカバリフレーズを
ユーザーが書き留めることに全面的に依存する。ところが**書き留めた内容が正しいかを確認する
手段が製品内に存在しなかった**。

BIP-39の導出は一方向なので、Otedamaは後からフレーズを再導出できない。つまり転記ミスは
**完全に無音**であり、検出する手段も、示唆するメトリクスも無く、露見する瞬間はただ一つ——
ディスクが既に壊れた後の復旧試行時である。約束の暗号的な部分（シードは端末を離れない）は
成立していたが、**約束を行使する能力**が欠けていた。これは磨き込みの前に埋めるべき穴である。

**追加した最小の部品.** `otedama wallet verify`:

- リカバリフレーズを**標準入力**から読む。argvからは決して読まない——コマンドライン引数は
  `/proc` と `ps` で全プロセスから見え、シェル履歴にも残る。ウォレット暗号化パスフレーズも
  同じ理由で `OTEDAMA_WALLET_PASSPHRASE` から取る（`run` が既に「フラグより推奨」と
  文書化している経路）。
- 比較対象は**64バイトのシードそのもの**であり、8桁のフィンガープリントではない。
  フィンガープリントは設計上公開値で、比較しても32ビット分しか証明できない。両シードは
  既にメモリ上にあるので、`crypto/subtle` での定数時間比較の方が強く、かつ無料。
  フィンガープリントは表示はする——`otedama doctor` および作成時の表示と突き合わせる値だから。
- **何も書かない.** ウォレットが存在しない場合は `lightning.NewWalletManager` の
  「新規作成」経路に入らせず、その前に停止する。verifyが検証対象を自分で作り出しては意味がない。
- 失敗原因を**2種類に区別**する。それぞれ対処が違うため: 無効なBIP-39ニーモニック
  （綴り間違い・単語の脱落——チェックサムが検出）と、有効だがこのウォレットのものではない
  フレーズ（語順違い、または `OTEDAMA_WALLET_MNEMONIC_PASSPHRASE` に25番目の単語が無い）。

**`internal/lightning` は一切変更していない.** 必要な部品——`MnemonicToEntropy`、
`MnemonicToSeed`、`Fingerprint`、`NewWalletManager`——はすべて既にエクスポート済みで、
新規コードは `cmd/otedama/wallet.go` に閉じている。CODEOWNERSゲート下の資金クリティカル領域に
未レビューの挙動変更を持ち込まない。

**意図的に含めなかったもの（②削除）.** `wallet change-passphrase` は同梱しなかった。
verifyは読み取り専用で、それ自体の根拠で出荷できる。一方ローテーションは `wallet.dat` を
書き換えるため、`internal/lightning` の atomic-replace 経路のメンテナレビューと共に出すべきである
（KNOWN_LIMITATIONS §16 に残件として記録）。

**検証.** 実際のウォレットを作って実バイナリで往復させた: `otedama run` が表示した24語を
`otedama wallet verify` に流して MATCH（フィンガープリント `bd69fda6` は `otedama doctor` の
報告と一致）、末尾1語を差し替えるとチェックサム違反で exit 1、別ウォレットの有効なフレーズでは
MISMATCH。ユニットテストは実 `WalletManager` で作成したウォレットに対して、一致・不一致・
25番目の単語の有無・大文字と乱れた空白・ウォレット不在時に作成しないこと・フレーズを
出力に復唱しないことを固定。

**副産物として発見・修正した実欠陥.** 新設の
`TestCompletion_ListsEveryDispatchedSubcommand`（completion.go自身のコメントが求める
「dispatchとの同期」を強制するテスト）が、`otedama completion --help` が **exit 64** を
返していたことを検出した。他の全サブコマンドは `parseSubcommandFlags` 経由で `--help` を
stdout + exit 0 に振り分けるが、completionはFlagSetを持たないためこの経路を通らず、
正しい呼び出しがスクリプトからは本物の誤用と区別できなかった——`parseSubcommandFlags` の
doc自身が「解消した」と書いている失敗モードそのものである。修正済み。

既存のcompletionテストはコマンド一覧の文字列リテラルを1本主張するだけで、
サブコマンド追加時に「省略」ではなく「リテラル」の理由で壊れ、リテラルを直せば
他の2シェルが未更新でも黙って通った。全シェル×全サブコマンドの双方向チェックに置き換えた。

全 24 パッケージが build・vet・test green。`cmd/otedama` のカバレッジ 91.6%。

### Removed / Fixed (session 264 — イーロン・マスクのアルゴリズム「①要件を疑う ②削除する」を適用: **シミュレーション市場を削除し、ダッシュボードの収益表示を実在する1つの数字に統一**)

**適用した方法.** マスクの5段階アルゴリズム——①すべての要件を疑う ②可能な部品・工程を削除する
③簡素化する ④サイクルタイムを短縮する ⑤最後に自動化する——の①②を製品定義に適用した。
「四系統にリアルタイム裁定配分する」という要件を疑うところから始めた。

**①要件を疑う.** 四系統のうち分散レンダリングと科学計算はv4.0スコープで着手禁止。
AI推論は「実装済み」とされていたが、`AkashProvider` はハードコードされた USD/hour 帯の
**中央値という定数**を返すだけで、Akash APIクライアントも入札もコンテナ管理も存在せず、
GPU計算ディスパッチ自体が存在しない（§4）。**四系統のうち実在するのは1つ**だった。

**②削除する — シミュレーション市場そのもの.** 「(simulated)」の接尾辞での開示ではなく削除を選んだ:

- そのクォートは**TUIの見出し数値「N sats/day」**と
  `otedama_arbitration_expected_yield_sats_per_second` に流入していた。接尾辞は
  プロバイダ一覧には出るが、**ユーザーが実際に読む大きな黄色の数字には何の印も付かない**。
  GPU搭載ホストでは、この架空の成分が実採掘収益を約5桁上回る。**誤った数字の隣の注意書きは、
  正しい数字とは別物である。**
- `CLAUDE.md` は「量産実装前の思弁的機能」を禁じており、定数を返す市場はまさにそれ。
- 「本物が来たとき用に残す」は何も保存しない。実APIに対して書くコードは定数と1行も共有しない。

**残したもの**（実統合が実際に必要とするすべて）: `Provider` インターフェース、`RateSource`、
`polling.go` のライフサイクル、そして任意個数のストリームを扱い複数ストリームでテスト済みの
裁定エンジン。

**②削除する — 収益表示の二重計上.** 削除の過程で、独立した実欠陥を発見した。
`tui.earningsLine` は次を**加算**していた:

1. `s.HashRate * defaultSatsPerHash()` ——ローカルな採掘収益モデル。
   ネットワークハッシュレートを **1e21 に凍結したハードコード定数**を持つ。
2. `Active` な全プロバイダの `SatsPerSecond` ——ここには**採掘プロバイダ自身が含まれる**。

つまり**実採掘収益が二重に計上**され、しかも2つの見積もりは難易度が動いた分だけ食い違う。
コメントは「Add AI inference yield from active providers」と書かれていたが、
コードは採掘を含む全プロバイダを加算していた。同じ量の実装が3つ存在していたことになる
（`MiningProvider` の収益モデル、`defaultSatsPerHash`、そして積分値 `EstSatsEarned`）。

**是正.** `defaultSatsPerHash` を削除し、見出しレートの出所を**1つ**にした——
裁定エンジンが実際にデバイスを割り当てているストリームの利回りの合計。結果として、
アイドルのデバイスは寄与せず、カーテイルメント中のマイナーは「本来の」レートではなく 0 を示し、
表示値は `otedama_arbitration_expected_yield_sats_per_second` と同じ数字を追う。

**テストが欠陥を隠していた.** 既存テストは `strings.Contains(line, "sats/day")` を
主張するだけで、二重計上でも通過した。厳密値テストに置き換えた
（1 sat/s の割当 → ちょうど `86400 sats/day`。二重計上ならこれを超える）。
割当ゼロなら `0 sats/day` を示すことも固定。

**併せて是正したドキュメント.**

- `BENCHMARKS.md` の経済比較表は "GPU AI (Akash) $12.00/day"、"Apple M2 Pro ~$8.00 (via NPU)"、
  および「裁定エンジンの価値の全ては GPU を AI推論へ回すことにある」という結論を載せていた。
  **いずれもコード経路として到達不能**。実装済みの系統のみの表に置き換え、GPU・ASIC 行が
  空である理由（§4、§8）を明記。
- `docs/architecture.md` は AI推論プロバイダについて「**Strawberry API**（並行開発中のプロジェクト）を
  主軸として統合」と記述していた。**そのようなAPIは実在せず**、`CLAUDE.md` 第七項
  「存在しないURL・API・エンドポイント・アドレスの記載」の禁止に該当する。削除し、
  統合先の選定基準（非カストディ・注文ごとの価格発見）の記述に置き換えた。
- `README.md` の日英両方、`docs/SPECIFICATION.md` G5、`docs/KNOWN_LIMITATIONS.md` §1
  （削除により解決）、`internal/hal` の関連コメント。

**削除した行数**: 本体 177 行 + テスト 551 行（`ai_inference.go`、`akash_test.go`、
`inference_yield_test.go`、`defaultSatsPerHash` とその周辺）。
...
`AkashProvider` の AI 推論収益と、**両者の比較そのもの** — を同じ方法で検算した。
比較は製品が存在する理由であり、どちらのプロバイダのテストもそれを扱っていなかった。

**変換式は正しい（検証済み）.** `SatsPerSecond(usdPerHour, btcUSD)` は
`(USD/h ÷ USD/BTC) × 1e8 ÷ 3600` で、単位変換として正しい。Akash の 20% 手数料は
`NetSatsPerSecond` に適用済み。GPU 不在時のゼロ収益・ゼロ信頼度クォートも
プロバイダ契約（沈黙せず発行し、裁定が回避できるようにする）どおり。

**発見1（doc の数値が約780倍過大）.** `AkashProvider` のパッケージ doc の
「GPU revenue comparison」表は RTX 4090 の Bitcoin SHA256d 採掘を
**~$0.05/day** としていた。本コードベース自身の定数（MiningProvider の GPU
推定 1.5 GH/s、ネットワーク ~1000 EH/s、補助金 3.125 BTC、$95k/BTC）から
再計算すると **$0.000064/day**（0.0675 sats/day）——**約780倍の過大**。
この誤りにより、推論:採掘の差が数百倍程度に見えていた（実際は**約17万倍**）。

**発見2（戦略の記述が算術的に成立しない）.** 上記の数値を前提に doc は
「AI 需要が高いときは GPU を Akash へ、低需要期は Bitcoin 採掘へ戻す」と
**双方向のスイッチ**を記述していたが、17万倍の差では**いかなる AI 価格でも
SHA256d GPU 採掘が勝つことはない**。さらに §4/§7 のとおり SHA256d 能力を
報告する GPU デバイスは存在せず、この分岐自体が到達不能である。

→ 実際に配分を動かしている要因に置き換えた: **2つのクォートは BTC 価格に対して
逆方向の感度を持つ**。採掘収益は sats 建てで価格に対して不変、推論収益は USD 建てなので
satoshi 価値は価格に**反比例**する。したがって BTC が上昇すると裁定は採掘側へ、
下落すると推論側へ移る。推論価格自体は固定中点（§1）なので、**これが今日唯一の
live なシグナル**である。

**発見3（比較のテストが無かった）.** 各クォートを個別に見るテストはあったが、
**比較そのもの**のテストは無く、推論クォートの桁も未検証だった。
→ `internal/provider/inference_yield_test.go` を新設し、変換式・手数料控除・
価格逆感度（10倍の値上がりで sats は1/10）・GPU 不在時の契約・
市場間比率の桁（意図的に動かした場合は doc の表も同時更新せよという指示つき）を固定。
**空振り防止**として、変換の 3600 を 60 に変えるとテストが `ratio 60` で失敗することを確認済み。
