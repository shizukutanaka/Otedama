...
### Fixed (session 254 — First Principles Thinkingで過不足機能を洗い出し改善: **リカバリフレーズがユーザーに一度も表示されていなかった**——非カストディの中核的約束の未履行を是正)
*(session 323: 別系チェーンの未マージ PR (#113) に留まっていた本 docs 更新を
現チェーンへ cherry-pick 移植。RESEARCH の項目解決は現チェーンの
session-293 版（chain-sdk TODO・SRI v1.11.0 pin・AEP-64 JWT 経路併記）を
優先して保持。)*
...
*(session 322: 別系チェーンの未マージ PR (#108) に留まっていた依存衛生更新を
現チェーンへ cherry-pick 移植。yaml インポート箇所は 2 ファイルのみで同一、
x/sys は本チェーンで既に wallet termios・TUI winsize が直接使用していたため
direct 化コメントをそのまま適合。)*
...
*(session 317: 別系チェーンの未マージ PR (#130) に留まっていた本機能を現チェーンへ
cherry-pick 移植。`IncomeMode`/`ThermalThrottleAboveCelsius` との Origins・
config show フィールド併記で統合、session-283 の共有ループ上の feed ポーリング
箇所は競合なく適合。)*
...
*(session 318: 別系チェーンの未マージ PR (#128) に留まっていた本機能を現チェーンへ
cherry-pick 移植。価格・サーマルに続く3番目のカーテルメントゲートとして
`applyCurtail` の OR に `carbonGate` を折り込み（炭素ゲート保持中に価格ゲートが
解除されても `otedama_curtailed` が 0 に偽装されないよう整合）、
`IncomeMode`/`ThermalThrottleAboveCelsius`/`ElectricityTariffOctopus` との
Origins・config show 併記で統合。)*
...
*(session 319: 別系チェーンの未マージ PR (#132) に留まっていた本修正を
現チェーンへ cherry-pick 移植。`runSession`/`dialPool` の dispatch に
`ProtocolDATUM` を既存 V1 経路へ併記、fakePool に deferredNotify /
handshake 応答を移植テスト側へ統合。)*### Fixed (session 254 — First Principles Thinkingで過不足機能を洗い出し改善: **リカバリフレーズがユーザーに一度も表示されていなかった**——非カストディの中核的約束の未履行を是正)
...
### Fixed (session 254 — First Principles Thinkingで過不足機能を洗い出し改善: **リカバリフレーズがユーザーに一度も表示されていなかった**——非カストディの中核的約束の未履行を是正)
*(session 320: 別系チェーンの未マージ PR (#111) に留まっていた本修正を
現チェーンへ cherry-pick 移植。`applyJob` が uint32 のジョブ ID を返す
シグネチャに統合し、タグ付けは共有 `dispatchJob` 内・判定は共有
`submitV1Share` 内に配線 — session-283 の poolproto 統合後でも同一経路を
通る。)*
